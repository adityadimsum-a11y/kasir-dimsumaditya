...
export default function HRDPayrollPage({ session, onSessionExpired, viewMode = "dashboard" }) {
  ...
  return <div className="da-page da-hrd-page-v4">
    ...
    <Modal open={Boolean(selectedEmployee)} title={selectedEmployee?.employee_name || "Profil Karyawan"} subtitle={`${selectedEmployee?.location_name || ""} · ${selectedEmployee?.position || ""}`} onClose={() => { setSelectedEmployee(null); setEmployeeProfile(null); }} size="xl">
      ...
      {!profileLoading && employeeProfile ? <>
        ...
        {detailTab === "payroll" && fullPayrollAccess ? <><h3 className="da-hrd-profile-section-title-v4">Payroll {profileYear}</h3><DataTable columns={[{key:"period",label:"Periode"},{key:"status",label:"Status",render:(row)=><Badge tone={badgeTone(row.status)}>{row.status}</Badge>},{key:"payment_status",label:"Bayar",render:(row)=><Badge tone={badgeTone(row.payment_status)}>{row.payment_status}</Badge>},{key:"bonus_amount",label:"Bonus",render:(row)=>formatRupiah(numberValue(row.bonus_amount))},{key:"overtime_amount",label:"Lembur",render:(row)=>formatRupiah(numberValue(row.overtime_amount))},{key:"net_pay",label:"THP",render:(row)=><strong>{formatRupiah(numberValue(row.net_pay))}</strong>}]} rows={profilePayroll} getRowKey={(row)=>row.payroll_run_id} /><h3 className="da-hrd-profile-section-title-v4">Pembayaran Gaji {profileYear}</h3><DataTable columns={[{key:"payment_date",label:"Tanggal",render:(row)=>formatDate(row.payment_date)},{key:"payment_id",label:"Payment ID"},{key:"wallet_name",label:"Dompet"},{key:"amount",label:"Dibayar",render:(row)=><strong>{formatRupiah(numberValue(row.amount))}</strong>}]} rows={profilePayments} getRowKey={(row)=>row.payment_id} /></> : null}
        {detailTab === "attendance" ? <DataTable columns={[{key:"attendance_date",label:"Tanggal",render:(row)=>formatDate(row.attendance_date)},{key:"attendance_type",label:"Jenis",render:(row)=><Badge tone={badgeTone(row.attendance_type)}>{row.attendance_type}</Badge>},{key:"day_fraction",label:"Hari",render:(row)=>numberValue(row.day_fraction)},{key:"deduct_salary",label:"Potong Gaji",render:(row)=>numberValue(row.deduct_salary) ? "Ya" : "Tidak"},{key:"overtime_amount",label:"Lembur",render:(row)=>formatRupiah(numberValue(row.overtime_amount))},{key:"notes",label:"Catatan"}]} rows={profileAttendance} getRowKey={(row)=>row.attendance_id} /> : null}
        {detailTab === "advance" ? <DataTable columns={[{key:"entry_date",label:"Tanggal",render:(row)=>formatDate(row.entry_date)},{key:"entry_type",label:"Jenis"},{key:"amount",label:"Nominal",render:(row)=>formatRupiah(numberValue(row.amount))},{key:"balance_effect",label:"Gerak Saldo",render:(row)=>formatRupiah(numberValue(row.balance_effect))},{key:"notes",label:"Catatan"}]} rows={profileAdvances} getRowKey={(row)=>row.advance_entry_id} /> : null}
        {detailTab === "loan" ? <><DataTable columns={[{key:"loan_date",label:"Tanggal",render:(row)=>formatDate(row.loan_date)},{key:"original_amount",label:"Awal",render:(row)=>formatRupiah(numberValue(row.original_amount))},{key:"remaining_amount",label:"Sisa",render:(row)=><strong>{formatRupiah(numberValue(row.remaining_amount))}</strong>},{key:"installment_amount",label:"Cicilan",render:(row)=>formatRupiah(numberValue(row.installment_amount))},{key:"status",label:"Status"}]} rows={profileLoans} getRowKey={(row)=>row.loan_id} /><h3 className="da-hrd-profile-section-title-v4">Gerakan Cicilan {profileYear}</h3><DataTable columns={[{key:"movement_date",label:"Tanggal",render:(row)=>formatDate(row.movement_date)},{key:"movement_type",label:"Jenis"},{key:"amount",label:"Nominal",render:(row)=>formatRupiah(numberValue(row.amount))},{key:"balance_after",label:"Sisa Setelah",render:(row)=>formatRupiah(numberValue(row.balance_after))},{key:"notes",label:"Catatan"}]} rows={profileLoanMovements} getRowKey={(row)=>row.loan_movement_id} /></> : null}
        {detailTab === "history" && fullPayrollAccess ? <><h3 className="da-hrd-profile-section-title-v4">Riwayat Gaji Master</h3><DataTable columns={[{key:"effective_period",label:"Berlaku"},{key:"salary_mode",label:"Sistem"},{key:"base_salary",label:"Gaji Pokok",render:(row)=>formatRupiah(numberValue(row.base_salary))},{key:"daily_salary",label:"Gaji Harian",render:(row)=>formatRupiah(numberValue(row.daily_salary))},{key:"source_type",label:"Sumber"}]} rows={profileSalary} getRowKey={(row)=>row.salary_history_id} /><h3 className="da-hrd-profile-section-title-v4">Riwayat Status</h3><DataTable columns={[{key:"effective_date",label:"Tanggal",render:(row)=>formatDate(row.effective_date)},{key:"from_status",label:"Dari"},{key:"to_status",label:"Menjadi"},{key:"reason",label:"Alasan"}]} rows={profileStatus} getRowKey={(row)=>row.status_history_id} /></> : null}
      </> : null}
    </Modal>
  </div>;
}
