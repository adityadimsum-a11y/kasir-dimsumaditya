...
const commonCss = `
  *{box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  .pill{display:inline-block;border-radius:999px;padding:5px 9px;background:#fee2e2;color:#b91c1c;font-weight:900;font-size:10px}
...
export function printPayrollSlipV32(row) {
    ...
    <main class="sheet">
      ...
      <section class="columns">
        <div class="box"><h2>Penghasilan</h2>
          ...
          <div class="line"><span>Tunjangan Tetap</span><b>${money(row?.fixed_allowance)}</b></div>
  ...
  `;
  const css = `${commonCss}
    @page{size:A4 portrait;margin:8mm}body{background:#fff}.sheet{width:194mm;min-height:130mm;margin:0 auto;padding:9mm;border:1px solid #d1d5db;border-radius:16px;page-break-after:${notice?.enabled && notice?.print_with_slip !== false ? "always" : "auto"}}
    header{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #ef2b22;padding-bottom:10px}.meta{text-align:right;display:grid;gap:6px}.identity{display:grid;grid-template-columns:1.4fr 1fr 1fr 1fr;gap:8px;margin:12px 0}.identity div{background:#f8fafc;border-radius:10px;padding:8px}.identity span{display:block;color:#64748b;font-size:9px;text-transform:uppercase;font-weight:900}.identity strong{display:block;margin-top:3px}
    .columns{display:grid;grid-template-columns:1fr 1fr;gap:10px}.box{border:1px solid #e5e7eb;border-radius:12px;overflow:hidden}.box h2{margin:0;padding:8px 10px;background:#fff1f2;color:#b91c1c;font-size:12px}.line{display:flex;justify-content:space-between;padding:5px 9px;border-bottom:1px dashed #e5e7eb}.line.total{background:#f8fafc;font-weight:900}.sub{font-size:9px;color:#64748b;padding:0 9px 5px}.loan{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:9px}.loan div{background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:7px}.loan span{display:block;font-size:9px;color:#92400e}.loan b{display:block;margin-top:2px}.thp{margin-top:9px;background:linear-gradient(135deg,#047857,#10b981);color:#fff;border-radius:13px;padding:10px 13px;display:grid;grid-template-columns:1fr auto;align-items:center}.thp span{font-weight:950}.thp strong{font-size:23px}.thp small{grid-column:1/3;opacity:.85}.notes{margin-top:8px;padding:7px 9px;background:#f8fafc;border-radius:9px}footer{margin-top:10px;text-align:center;color:#64748b;font-size:9px;font-weight:700}
    @media print{.sheet{border:0;border-radius:0;padding:5mm}}
    ${absenceNoticeCss()}
  `;
