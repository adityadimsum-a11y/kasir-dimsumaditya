...
import { getKasDompetBootstrap } from "../../lib/api/actions";
...
function normalizeMutation(row) {
  ...
  return {
    ...
    wallet_id: row.wallet_id || row.account_id || "",
...
export default function KasDompetPage({ session, onSessionExpired }) {
  ...
  const wallets = useMemo(() => {
    return asArray(bootstrap?.wallets).map(normalizeWallet);
  }, [bootstrap]);

  const mutations = useMemo(() => {
    return asArray(bootstrap?.wallet_mutations).map(normalizeMutation);
  }, [bootstrap]);

  const filteredMutations = useMemo(() => {
    return filterMutations(mutations, activeTab, selectedWalletId);
  }, [mutations, activeTab, selectedWalletId]);
  ...
  const loadData = async () => {
    ...
    const result = await getKasDompetBootstrap(session?.sessionToken, {
      source: "frontend_part_4c_kas_dompet_mutasi_uang",
    });
  ...
  const mutationColumns = [
    ...
    },
    {
      key: "direction",
      label: "Arah",
      render: (row) => <Badge tone={getDirectionTone(row.direction, row.signed_amount)}>{row.direction_label}</Badge>,
    },
    {
      key: "amount",
      label: "Nominal",
    ...
    },
    {
      key: "status",
      label: "Status",
      render: (row) => <Badge tone="warning">{safeText(row.status)}</Badge>,
    },
  ];
  ...
  return (
    <div>
      <PageHeader
        title="Kas & Dompet"
        description="Pantau saldo cash, BCA, BRI, dan mutasi uang hidup dari transaksi aktual. Ini dasar sebelum masuk 4 Amplop."
        badge="Read Only"
      />
      ...
      <div className="da-dashboard-banner">
        <div>
          ...
          <div className="da-dashboard-banner-desc">
            Halaman ini hanya membaca uang aktual dari mutasi dompet. 4 Amplop nanti tetap tidak boleh mengambil dari PO, piutang, atau stok.
          </div>
        </div>
      ...
      <div className="da-grid da-grid-3">
        <StatCard
          ...
          description="Saldo hitung dari mutasi uang masuk dan keluar."
        />
        <StatCard
          label="Uang Masuk"
          value={loading ? "..." : formatRupiah(summary.total_in)}
          description="Total mutasi uang masuk aktual."
        />
        <StatCard
          tone="warning"
          label="Uang Keluar"
          value={loading ? "..." : formatRupiah(summary.total_out)}
          description="Total mutasi keluar dari kas/dompet."
        />
      </div>
      ...
      <div className="da-grid da-grid-3">
        <StatCard
          label="Dompet Aktif"
          value={loading ? "..." : summary.wallet_count}
          description="Jumlah dompet yang terbaca."
        />
        <StatCard
          label="Mutasi Tercatat"
          value={loading ? "..." : summary.mutation_count}
          description="Jumlah mutasi uang masuk/keluar."
        />
        <StatCard
          tone="primary"
          label="Bahan 4 Amplop"
          value={loading ? "..." : formatRupiah(summary.amplop_ready)}
          description="Masih read-only. Nanti hanya dari uang masuk aktual."
        />
      </div>
      ...
      <Card>
        <div className="da-section-heading">
          <div>
            ...
            <p className="da-muted">
              Klik kartu dompet untuk filter mutasi. Klik lagi Semua Dompet untuk reset.
            </p>
          </div>
        ...
        <div className="da-action-grid">
          {wallets.length === 0 ? (
            <div className="da-muted">Belum ada dompet terbaca.</div>
          ) : (
            wallets.map((wallet) => (
              <WalletCard
                key={wallet.wallet_id || wallet.wallet_name}
                wallet={wallet}
                selected={selectedWalletId === wallet.wallet_id}
      ...
      <Card>
        <div className="da-section-heading">
          <div>
            ...
            <p className="da-muted">
              Semua baris harus punya sumber transaksi supaya uang bisa ditelusuri ke order, payment, kas keluar, atau hutang.
            </p>
          </div>
        ...
        <div className="da-tabs">
          <button
            type="button"
            className={activeTab === "all" ? "da-tab active" : "da-tab"}
          ...
          <button
            type="button"
            className={activeTab === "in" ? "da-tab active" : "da-tab"}
          ...
          <button
            type="button"
            className={activeTab === "out" ? "da-tab active" : "da-tab"}
        ...
        <DataTable
          ...
          onRowClick={setSelectedMutation}
        />
      </Card>

      <Modal
        open={Boolean(selectedMutation)}
        title="Detail Mutasi Uang"
        subtitle={selectedMutation?.mutation_id || ""}
          ...
          <div>
            ...
            <div className="da-detail-grid">
              ...
              <div className="da-detail-box">
                ...
                <p><strong>Catatan:</strong> Detail lanjut nanti dibuka dari Arsip Digital / source modal.</p>
              </div>
            </div>

            <div className="da-modal-note" style={{ marginTop: 14 }}>
              Uang masuk aktual dari mutasi ini bisa menjadi bahan 4 Amplop. Piutang/PO/stok tidak boleh masuk amplop sebelum berubah jadi uang masuk nyata.
            </div>
          </div>
        ) : null}
      </Modal>
