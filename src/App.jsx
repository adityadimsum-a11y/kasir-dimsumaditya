...
const CACHE_TTL = 60000; // 60 Detik Memory Cache Layer
...
export default function App() {
  ...
  const processOfflineQueue = async () => {
      ...
      if(newQueue.length === 0) showToast("✅ Semua antrean offline disinkronisasi.", 'success');
      fetchData(true);
  };
  ...
  const sendToSheet = async (action, payloadData, table) => {
    ...
    if (isOffline) { 
        ...
        showToast("⚠️ OFFLINE: Transaksi masuk antrean latar belakang.", 'error'); 
        return true; 
    }

    if (isLoading) { showToast("⏳ Mencegah double click, mohon tunggu...", 'error'); return false; }
    setIsLoading(true);
    ...
    try { 
        ...
        showToast(`✅ Data Terpelihara Di Jurnal & Ledger.`, 'success');
        await fetchData(true); 
        return true;
    } catch (error) { 
        offlineQueueRef.current.push(requestPayload);
        window.localStorage.setItem('erp_offline_queue', JSON.stringify(offlineQueueRef.current));
        showToast("🚨 Jaringan bermasalah. Masuk antrean lokal.", 'error'); 
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50/80 backdrop-blur-sm z-50 fixed inset-0">
        <div className="bg-white p-6 rounded-2xl shadow-xl flex flex-col items-center border border-slate-100">
          ...
          <div className="text-xs font-bold text-slate-600 tracking-widest uppercase">Initial Enterprise Sync...</div>
        </div>
      </div>
    );
  }
