    ...
    <div className="min-h-screen bg-slate-50 flex">
      <aside className="w-64 bg-slate-900 text-white flex flex-col shrink-0 relative shadow-xl z-20">
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto custom-scrollbar">
            ...
            <NavItem icon={<Send size={20} />} label="Delivery Order (DO)" active={activeTab === 'distribusi'} onClick={() => handleTabChange('distribusi')} />
    ...
    <div className="min-h-screen bg-slate-50 flex">
      <aside className="w-64 bg-slate-900 text-white flex flex-col shrink-0 relative shadow-xl z-20">
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto">
            ...
            <NavItem icon={<Package size={20} />} label="Stok & Terima Barang" active={activeTab === 'stok'} onClick={() => handleTabChange('stok')} />
...
export default function App() {
  ...
  const handleLogin = (e) => {
    ...
    // BACKWARD COMPATIBILITY SEMENTARA JIKA TABEL KOSONG
  ...
  const sendToSheet = async (action, data, table) => {
    // ... logic optimistik state (bisa menggunakan yang ada, di-bypass ke backend untuk keamanan)
    ...
    setTimeout(fetchData, 1500); // Reload data to ensure sync with Finance Engine
  };
  ...
  const globalProps = {
    ...
    data: { 
        ...
        supplierLedger, cashflowTransactions, marketplaceSettlement // PASS KE DASHBOARD
    }
  };
