...
export function buildPrintBackupReport({ healthData, actionHubData, backupData, session }) {
  ...
  const rows = [
    makeSafetyRow({
      id: "proxy-json",
      title: "Kabel Apps Script & Proxy",
      detail: healthData ? "Proxy dan Apps Script sudah membalas JSON untuk laporan." : "Belum ada response Data Health.",
    ...
    makeSafetyRow({
      ...
      detail: "Sebelum dipakai full staff, owner tetap perlu simpan salinan Google Sheet / export file penting secara manual.",
      status: "PERLU UAT",
      tone: "warning",
      blocker: false,
      source: "Manual Owner",
