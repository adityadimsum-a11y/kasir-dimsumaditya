...
export function getGoLiveReadiness({ healthData, actionHubData } = {}) {
  ...
  const checks = [
    makeCheck({
      id: "backend-json",
      title: "Apps Script & Proxy membalas JSON",
      detail: healthData
        ? "Kabel Vercel proxy dan Apps Script sudah membalas data JSON."
    ...
    makeCheck({
      ...
      detail: `${health.ghost} ghost/formatting row terdeteksi. Boleh dirapikan dari sheet sumber, tapi tidak dihitung sebagai transaksi hidup.`,
