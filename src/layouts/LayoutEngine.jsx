...
export default function LayoutEngine({ user, activeTab, setActiveTab, handleLogout, masterCapabilities, children }) {
  ...
  return (
    <div className="min-h-screen bg-transparent flex overflow-hidden font-sans">
      ...
      {/* 🔥 SIDEBAR: Dikembalikan jadi Solid Mewah (Hitam Pekat) */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-950 border-r border-slate-800 text-slate-300 transition-transform duration-300 ease-in-out flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:flex-shrink-0 shadow-2xl`}>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950">
          <div className="flex items-center gap-3 px-3 py-2 mb-3 bg-slate-900 rounded-xl border border-slate-800">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <div className="text-[10px] font-bold text-slate-500 uppercase truncate">{user?.branch_id || 'NODE'}</div>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden relative">
        ...
        {/* 🔥 FROSTED GLASS BACKGROUND (Bersih & Elegan) */}
