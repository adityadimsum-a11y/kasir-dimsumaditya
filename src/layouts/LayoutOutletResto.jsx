...
import { 
  ...
  Menu, X, ShieldCheck, Clock, Lock, Package
} from 'lucide-react';
...
export default function LayoutOutletResto({ user, activeTab, setActiveTab, handleLogout, children }) {
  ...
  // =====================================
  // STRUKTUR MENU OUTLET RESTO (PHASE 12.5)
  // =====================================
  const menuGroups = [
    {
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 z-30 relative shadow-sm">
          <div className="flex items-center gap-4">
            <button onClick={() => setIsMobileMenuOpen(true)} className="lg:hidden p-2 -ml-2 text-slate-600 hover:bg-slate-100 rounded-lg transition">
              <Menu size={24} />
            </button>
            <div>
