...
// PERBAIKAN A4 STYLE: PADDING DIPERKECIL & FONT 10PX AGAR MUAT BANYAK
const a4Style = `
      ...
      <div className="print-wrapper shadow-xl">
        ...
        <div className="flex justify-between items-start mt-4">
            <div className="flex-1 mr-6">
                <div className="mb-3"><span className="text-[9px] font-bold uppercase text-slate-500 block mb-0.5">Terbilang :</span><span className="font-bold italic text-[11px]"># {terbilang(data.totalAll)} Rupiah #</span></div>
            ...
            <div className="w-56">
                <div className="flex justify-between mb-1.5 text-xs"><span className="font-bold uppercase text-slate-600">Subtotal</span><span className="font-black">{formatRp(data.totalAll)}</span></div>
                ...
                <div className="flex justify-between border-t-2 border-black pt-1.5 mt-1.5"><span className="font-black text-sm uppercase">SISA TAGIHAN</span><span className="font-black text-sm">{formatRp(Number(data.totalAll) - Number(data.paidAmount))}</span></div>
            </div>
        </div>
    ...
    <div className="bg-slate-100 min-h-screen p-4">
      ...
      <div className="print-wrapper shadow-xl">
        ...
        <div className="flex justify-end mt-4">
          <div className="w-64">
            <div className="flex justify-between mb-1.5 text-xs"><span className="font-bold uppercase text-slate-600">Total Belanja</span><span className="font-black">{formatRp(data.totalAll)}</span></div>
            <div className="flex justify-between mb-1.5 text-xs"><span className="font-bold uppercase text-slate-600">Telah Dibayar</span><span className="font-bold">{formatRp(data.paidAmount)}</span></div>
            <div className="flex justify-between border-t-2 border-black pt-1.5 mt-1.5">
                ...
                <span className="font-black text-sm text-red-600">{formatRp(Number(data.totalAll) - Number(data.paidAmount))}</span>
            </div>
          </div>
        </div>
...
// ============================================================================
// KOMPONEN PRINT LAPORAN REKAP PUSAT (A4) - COMPACT SPACING
// ============================================================================
  ...
  return (
    <div className="bg-slate-100 min-h-screen p-4">
      ...
      <div className="a4-wrapper shadow-xl border border-gray-200">
        ...
        {/* SUMMARY DIBUAT LEBIH KECIL PADDINGNYA */}
        ...
        <table className="table-print">
          <thead>
            <tr><th className="w-8">NO</th><th>TGL & INV</th><th>PELANGGAN</th><th className="text-center">VIA</th><th className="text-right">TAGIHAN</th><th className="text-right">TERBAYAR</th><th className="text-right">SISA</th><th className="text-center">STATUS</th></tr>
          </thead>
          <tbody>
            {(!rekap?.listTransaksiDetail || rekap.listTransaksiDetail.length === 0) ? (
                <tr><td colSpan="8" className="text-center py-4 italic text-slate-500">Tidak ada transaksi di periode ini.</td></tr>
            ) : (
            ...
            <tr>
                <td colSpan="4" className="text-right font-bold uppercase bg-slate-50">Total Omset Penjualan :</td>
              ...
              <tbody>
                {rekap.listRiwayatPiutang.map((p, i) => (
                  <tr key={i}>
                    ...
                    <td className="text-center">{p.qtyDesc}</td>
        ...
        {rekap?.listExpenses?.length > 0 && (
          <>
            <h3 className="font-bold text-xs mb-1.5 mt-5 text-slate-800">D. BUKU KAS (PENGELUARAN)</h3>
            <table className="table-print">
              <thead>
...
// ============================================================================
// KOMPONEN PRINT LAPORAN CABANG PEMALANG (A4) - COMPACT SPACING
// ============================================================================
  ...
  return (
    <div className="bg-slate-100 min-h-screen p-4">
      ...
      <div className="a4-wrapper shadow-xl border border-gray-200">
        ...
        <table className="table-print">
          <thead><tr><th className="w-8">NO</th><th>TGL & INV</th><th>PELANGGAN</th><th className="text-center">VIA</th><th className="text-right">TAGIHAN</th><th className="text-right">TERBAYAR</th><th className="text-right">SISA</th><th className="text-center">STATUS</th></tr></thead>
          <tbody>
            {(!rekap?.listOrders || rekap.listOrders.length === 0) ? (
                <tr><td colSpan="8" className="text-center py-4 italic text-slate-500">Tidak ada transaksi penjualan cabang.</td></tr>
            ) : (
            ...
            <tr>
                <td colSpan="4" className="text-right font-bold uppercase bg-slate-50">Total Omset Cabang :</td>
                <td className="text-right font-black text-blue-700 bg-slate-50">{formatRp(rekap?.totalPenjualanKotor)}</td>
                <td colSpan="3" className="bg-slate-50"></td>
            </tr>
          </tbody>
        ...
        <h3 className="font-bold text-xs mb-1.5 mt-5 text-slate-800">C. LAPORAN HARIAN & STOK</h3>
        <table className="table-print">
