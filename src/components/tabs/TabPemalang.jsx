...
export default function TabPemalang({ 
  ...
  const [form, setForm] = useState({
    actual_cash: '',
    transfer_amount: '',
    transfer_method: 'BCA_PUSAT',
    notes: ''
  });

  // ========================================================
  // CALCULATION ENGINE: REKAPITULASI HARIAN NODE CABANG
  // ========================================================
  const dailyMetrics = useMemo(() => {
    ...
    const isClosed = (financial_closings || []).some(c => 
      ...
      (!c.isDeleted || String(c.isDeleted).toUpperCase() !== 'TRUE')
    );
  ...
  // ========================================================
  // SUBMIT CLOSING & SETTLEMENT KE PUSAT
  // ========================================================
  const handleClosingSubmit = async (e) => {
    e.preventDefault();

    if (dailyMetrics.isClosed) {
      alert("Cabang ini sudah melakukan Closing hari ini!");
      return;
    }

    const confirmMsg = `Konfirmasi SETORAN KE PUSAT:\n\nTotal Disetor: ${formatRp(form.transfer_amount)}\n\nApakah Anda yakin nominal sudah ditransfer? Data akan dikunci.`;
    ...
    const closingPayload = {
      ...
      cash_ready: actualCashNum,
      total_sales: dailyMetrics.cashSales + dailyMetrics.marketplaceAR,
      status: 'LOCKED',
    ...
    const settlementPayload = {
      ...
      branch_id: currentBranch,
      period: todayStr,
      cash_collected: actualCashNum,
      amount_to_transfer: transferAmt,
      amount_transferred: transferAmt,
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="space-y-4">
              <div>
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Penjualan Tunai / QRIS</div>
                <div className="text-lg font-black text-emerald-600">{formatRp(dailyMetrics.cashSales)}</div>
              </div>
              <div>
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Piutang Mengambang Marketplace</div>
                <div className="text-lg font-black text-orange-500">{formatRp(dailyMetrics.marketplaceAR)}</div>
              </div>
              <div className="border-b border-dashed border-slate-200 pb-4">
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Pengeluaran Cabang (Opex)</div>
                <div className="text-lg font-black text-rose-500">- {formatRp(dailyMetrics.totalExpenses)}</div>
              </div>
              <div className="pt-2 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <div className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1">Ekspektasi Kas Laci (Harusnya Ada)</div>
                <div className="text-2xl font-black text-slate-800">{formatRp(dailyMetrics.expectedCash)}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className={`bg-white rounded-3xl border shadow-sm overflow-hidden ${dailyMetrics.isClosed ? 'opacity-70 pointer-events-none' : 'border-blue-200'}`}>
            <div className={`p-4 border-b ${dailyMetrics.isClosed ? 'bg-slate-50' : 'bg-blue-50'} flex items-center justify-between`}>
               <h4 className="font-black text-slate-800 tracking-widest uppercase text-xs flex items-center gap-2">
                 <Send size={16} className={dailyMetrics.isClosed ? 'text-slate-400' : 'text-blue-600'}/> 
                 Form Validasi & Setor Ke Pusat
               </h4>
            </div>
            ...
            <form onSubmit={handleClosingSubmit} className="p-6 md:p-8 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      type="text" 
                      required 
                      value={form.actual_cash ? Number(form.actual_cash).toLocaleString('id-ID') : ''} 
                      onChange={e => setForm({...form, actual_cash: e.target.value.replace(/\D/g, '')})} 
                      className="w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl font-black text-slate-800 outline-none focus:ring-2 focus:ring-blue-500" 
                      placeholder="0" 
                    />
                  </div>
                  {/* Indikator Selisih */}
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      type="text" 
                      required 
                      value={form.transfer_amount ? Number(form.transfer_amount).toLocaleString('id-ID') : ''} 
                      onChange={e => setForm({...form, transfer_amount: e.target.value.replace(/\D/g, '')})} 
                      className="w-full pl-11 pr-4 py-3.5 bg-blue-50 border border-blue-200 rounded-2xl font-black text-blue-800 outline-none focus:ring-2 focus:ring-blue-500" 
                      placeholder="0" 
                    />
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Rekening Tujuan Pusat</label>
                  <select 
                    value={form.transfer_method} 
                    onChange={e => setForm({...form, transfer_method: e.target.value})} 
                    className="w-full p-3.5 bg-white border border-slate-200 rounded-2xl text-xs font-bold outline-none"
                  >
                    <option value="BCA_PUSAT">BCA Pusat (Operasional)</option>
                    <option value="MANDIRI_PUSAT">Mandiri Pusat</option>
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full p-3.5 bg-white border border-slate-200 rounded-2xl text-xs font-bold outline-none" 
                    placeholder="Contoh: Titip lewat supir DO" 
                  />
                </div>
              </div>
              ...
              <button 
                type="submit" 
                disabled={dailyMetrics.isClosed}
                className="w-full bg-slate-900 hover:bg-slate-800 text-white font-black py-4 rounded-2xl uppercase tracking-widest text-xs flex justify-center items-center gap-2 mt-4 shadow-lg disabled:opacity-50 transition"
              >
                {dailyMetrics.isClosed ? <><Lock size={16}/> Node Sudah Terkunci Hari Ini</> : <><Send size={16}/> Kunci Data & Kirim Setoran ke Pusat</>}
              </button>
            </form>
          </div>
        </div>
            ...
            <table className="w-full text-sm text-left">
               <thead className="text-[10px] text-slate-400 uppercase tracking-widest border-b border-slate-100">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-center">Metode Transfer</th>
                    <th className="px-6 py-4 text-right">Kas Real Terkumpul</th>
                    <th className="px-6 py-4 text-right">Nominal Disetor (Settled)</th>
                    <th className="px-6 py-4 text-center">Status</th>
                  </tr>
               </thead>
               <tbody className="divide-y divide-slate-100 text-xs font-bold">
                  {mySettlements.length === 0 ? (
                      <tr><td colSpan="5" className="text-center py-8 text-slate-400">Belum ada histori setoran ke pusat.</td></tr>
                  ) : (
                      mySettlements.slice(0, 10).map(s => (
                         <tr key={s.settlement_id} className="hover:bg-slate-50 transition">
                            <td className="px-6 py-4">
                              <div className="text-slate-800">{formatDate(s.transfer_date || s.period)}</div>
                              <div className="text-[9px] text-slate-400 font-mono mt-0.5">{s.settlement_id}</div>
                            </td>
                            <td className="px-6 py-4 text-center">
                              <span className="px-2 py-1 rounded-lg bg-slate-100 text-slate-600 text-[9px] uppercase tracking-wider">{s.transfer_method}</span>
                            </td>
                            <td className="px-6 py-4 text-right text-slate-500">{formatRp(s.cash_collected)}</td>
                            <td className="px-6 py-4 text-right text-blue-600 font-black text-sm">{formatRp(s.amount_transferred)}</td>
                            <td className="px-6 py-4 text-center">
                                <span className="flex items-center justify-center gap-1 text-emerald-600 text-[10px] uppercase tracking-wider bg-emerald-50 px-2 py-1 rounded-lg w-max mx-auto">
