...
import { getTodayStr, getLocalYMD, formatRp, parseRp, generateId, safeSort, formatDate, SATUAN_BARANG } from '../../utils/helpers';
...
export default function TabPurchases({ purchases, payments, sendToSheet, requestDelete, setPrintData }) {
  ...
  const updateCartItem = (index, field, value) => {
      const newCart = [...cart]; newCart[index][field] = value; setCart(newCart);
  ...
  const handleEdit = (item) => {
    ...
    setDate(String(item.date).split('T')[0]); setSupplier(item.supplier); setPaymentMethod(item.paymentMethod); 
  ...
  const displayPurchases = useMemo(() => {
    const filtered = (purchases||[]).filter(p => { const ymd = getLocalYMD(p?.date); return ymd && ymd >= filterFrom && ymd <= filterTo; });
    const groups = {};
    filtered.forEach(p => {
        if(!p?.id) return;
  ...
  return (
    <div className="space-y-4 animate-in fade-in">
      <div className="flex justify-between items-center">
        <div><h3 className="font-bold text-lg text-slate-800">Pembelian Bahan Baku (Restock)</h3></div>
        <button onClick={() => { if(showForm) resetForm(); else setShowForm(true); }} className={`px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-medium text-white ${showForm ? 'bg-slate-500' : 'bg-orange-600'}`}>{showForm ? <X size={16} /> : <Plus size={16} />} {showForm ? 'Batal' : 'Catat Pembelian Baru'}</button>
      </div>
        ...
        <form onSubmit={handleSimpan} className="bg-white p-6 rounded-xl border border-orange-200 shadow-sm grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-3 bg-orange-50 p-4 rounded-xl border border-orange-100">
             ...
             <div className="space-y-3">
                 {(cart||[]).map((item, index) => {
                     return (
                     <div key={index} className="flex gap-2 items-start bg-white p-2 rounded-lg border relative pr-8">
                         <div className="w-5/12 space-y-1"><input type="text" list="item-list" required placeholder="Nama Barang" value={item.itemName} onChange={e=>updateCartItem(index, 'itemName', e.target.value)} className="w-full p-2 border rounded text-xs uppercase font-bold" /><datalist id="item-list">{listBarangUnik.map(b => <option key={b} value={b} />)}</datalist></div>
                         <div className="w-2/12 space-y-1"><input type="number" min="1" required placeholder="Qty" value={item.qty} onChange={e=>updateCartItem(index, 'qty', e.target.value)} className="w-full p-2 border rounded text-xs font-bold text-center" /></div>
                         <div className="w-2/12 space-y-1"><input type="text" list="satuan-list" required placeholder="Satuan" value={item.satuan} onChange={e=>updateCartItem(index, 'satuan', e.target.value)} className="w-full p-2 border rounded text-xs uppercase" /><datalist id="satuan-list">{SATUAN_BARANG.map(b=><option key={b} value={b}/>)}</datalist></div>
                         <div className="w-3/12 space-y-1"><input type="text" required placeholder="Harga" value={formatRp(item.price)} onChange={e=>updateCartItem(index, 'price', parseRp(e.target.value))} className="w-full p-2 border rounded text-xs font-bold" /></div>
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {displayPurchases.map((pur) => {
              ...
              <tr key={pur.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3 text-center">{sisaHutang > 0 ? <span className="bg-red-100 text-red-700 px-2 py-1 rounded text-[10px] font-bold">HUTANG</span> : <span className="bg-emerald-100 text-emerald-700 px-2 py-1 rounded text-[10px] font-bold">LUNAS</span>}</td>
                <td className="px-4 py-3 text-center"><div className="flex justify-center gap-2"><button onClick={() => setPrintData({ type: 'purchase', data: pur })} className="text-slate-600 bg-slate-100 p-2 rounded-lg" title="Cetak"><Printer size={16} /></button><button onClick={() => handleEdit(pur)} className="text-blue-600 bg-blue-50 px-2 py-1 rounded-lg font-bold text-[10px]">EDIT</button><button onClick={() => requestDelete(pur.id)} className="text-red-500 bg-red-50 p-2 rounded-lg"><Trash2 size={16} /></button></div></td>
              </tr>
            )})}
          </tbody>
        </table>
