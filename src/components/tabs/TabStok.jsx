...
import { Layers, CheckCircle, AlertTriangle, Scale, CookingPot, Package } from 'lucide-react';
import { formatRp, getTodayStr, generateId } from '../../utils/helpers';

export default function TabStok({ productionBatches, stockMovements, inventoryCostLayers, sendToSheet, showToast, user }) {
  const todayStr = getTodayStr();
  
  // FORM BATCH PRODUKSI (PHASE 12.5 ENGINE SEED)
  const [formProd, setFormProd] = useState({
     date: todayStr, adukan_used: 1, result_pcs: 1000, overhead_cost: 50000, notes: ''
  });

  const totalAyamCalculated = Number(formProd.adukan_used) * 30; // 1 Adukan = 30 KG
  const targetPcsStandard = Number(formProd.adukan_used) * 1000; // 1 Adukan = 1000 Pcs
  const currentYield = targetPcsStandard > 0 ? (Number(formProd.result_pcs) / targetPcsStandard) * 100 : 0;

  // CONVERSION MATRICES VIEW (Ayam -> Pcs -> Mika -> Porsi)
  const autoConversion = useMemo(() => {
     const pcs = Number(formProd.result_pcs) || 0;
     return {
        mika: Math.ceil(pcs / 50),     // 1 Mika = 50 Pcs
        porsi: Math.ceil(pcs / 4),     // 1 Porsi = 4 Pcs
        kantong: (totalAyamCalculated / 10).toFixed(1) // 1 Kantong = 10 KG
     };
  ...
  const handleCreateProduction = (e) => {
     ...
     sendToSheet('event_production', payload, 'system_events').then(success => {
         if(success) setFormProd({ date: todayStr, adukan_used: 1, result_pcs: 1000, overhead_cost: 50000, notes: '' });
     });
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
         {/* PRODUCTION FORM */}
         <div className="bg-white rounded-2xl border shadow-sm p-6 h-max">
            ...
            <form onSubmit={handleCreateProduction} className="space-y-4">
               ...
               <div><label className="text-[10px] font-bold text-slate-500 uppercase">Biaya Overhead Pembantu (Gas/Bumbu/Packaging)</label><input type="number" required value={formProd.overhead_cost} onChange={e=>setFormProd({...formProd, overhead_cost:e.target.value})} className="w-full p-2.5 bg-slate-50 border rounded-xl font-bold text-sm" /></div>
         ...
         {/* LIVE YIELD CALCULATOR & AUTOMATIC CONVERSION */}
