...
export default function TabStok({ 
  productionBatches = [], production_batches, purchases = [], purchases_data,
  karyawan = [], masterBranches = [], master_branches, sendToSheet, showToast, user, requestDelete 
}) {
  ...
  // KALKULASI SISA STOK AYAM GUDANG
  const stokAyamGudang = useMemo(() => {
    let ayamMasuk = 0; let ayamKeluar = 0;
    realPurchases.filter(p => !p.isDeleted && p.category === 'BAHAN_BAKU' && (p.branch_id === currentBranch || p.branch_id === 'PUSAT')).forEach(p => { ayamMasuk += Number(p.qty_kg || 0); });
    realProductionBatches.filter(p => !p.isDeleted && (p.branch_id === currentBranch || p.branch_id === 'PUSAT')).forEach(p => { ayamKeluar += Number(p.total_ayam_kg || 0); });
    return Math.max(0, ayamMasuk - ayamKeluar);
  }, [realPurchases, realProductionBatches, currentBranch]);

  const kalkulasiOtomatis = useMemo(() => {
    const qty = Number(form.adukan || 0);
    return { ayamKg: qty * BOM.KG_AYAM_PER_ADUKAN, ayamKantong: qty * BOM.KANTONG_AYAM_PER_ADUKAN, hasilPcs: qty * BOM.PCS_YIELD_PER_ADUKAN, hasilMika: qty * BOM.MIKA_YIELD_PER_ADUKAN, hasilPorsi: qty * BOM.PORSI_YIELD_PER_ADUKAN, hppTotal: qty * BOM.HPP_AYAM_PER_ADUKAN };
  }, [form.adukan]);
  ...
  const handleSubmit = async (e) => {
    ...
    // 🔥 GUARDRAIL: BLOKIR JIKA AYAM GUDANG KOSONG
    if (!isEditing && kalkulasiOtomatis.ayamKg > stokAyamGudang) {
      return alert(`❌ GAGAL! Stok Ayam Mentah di Gudang tidak mencukupi!\nSisa Gudang: ${stokAyamGudang} Kg\nKebutuhan Adukan: ${kalkulasiOtomatis.ayamKg} Kg`);
    }
  ...
  const handleEdit = (log) => {
    setForm({ id: log.id, date: log.date.split('T')[0], picId: log.pic_id || '', adukan: String(log.total_adukan || 0), notes: log.notes || '' });
  ...
  const handleDelete = async (id) => {
    if(window.confirm("AWAS! Menghapus data ini akan merusak laporan stok gudang. Yakin ingin menghapus?")) {
      if(requestDelete) { requestDelete(id); } else {
        setOptimisticDeletedIds(prev => { const n = new Set(prev); n.add(id); return n; });
        if(await sendToSheet('delete', { id }, 'production_batches')) { if(showToast) showToast('Data produksi divoid.', 'success'); } 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border shadow-sm border-l-4 border-l-blue-500 relative overflow-hidden">
          ...
          <div className="flex justify-between items-start relative z-10">
            <div>
              <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Ayam Dipakai Hari Ini</div>
              <div className="text-2xl font-black text-blue-600 mt-1">{formatNumber(metrikRadar.ayamKgHariIni)} <span className="text-xs">KG</span></div>
            </div>
          </div>
          {/* INDIKATOR STOK GUDANG */}
          <div className="text-[9px] font-bold mt-2 pt-2 border-t border-slate-100 flex items-center justify-between">
            <span className="text-slate-500 uppercase">Sisa Gudang:</span>
            <span className={`px-2 py-0.5 rounded font-black ${stokAyamGudang <= 0 ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-700'}`}>{formatNumber(stokAyamGudang)} KG</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={`p-6 rounded-2xl border transition-all h-max shadow-sm ${isEditing ? 'bg-amber-50/50 border-t-4 border-t-amber-500 border-amber-200' : 'bg-white border-t-4 border-t-emerald-600'}`}>
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              ...
              {isEditing && (
                <button type="button" onClick={() => { setIsEditing(false); setForm({ id: '', date: todayStr, picId: '', adukan: '1', notes: '' }); }} className="text-[10px] border px-2.5 py-1 rounded-lg font-black uppercase text-slate-500 bg-white shadow-sm flex items-center gap-1 hover:bg-slate-50"><Undo size={12}/> Batal</button>
              )}
            </div>
            ...
            <div className="p-5 bg-emerald-50/50 border border-emerald-200 rounded-2xl shadow-inner">
              ...
              <div className="mt-5 pt-4 border-t border-emerald-200/60 grid grid-cols-2 gap-3">
                <div className="bg-white p-3 rounded-xl border border-rose-100 shadow-sm">
                  ...
                  <div className="text-[10px] font-bold text-slate-500 mt-1 bg-slate-50 px-2 py-1 rounded inline-block border border-slate-100">Konversi: {formatNumber(kalkulasiOtomatis.ayamKantong)} Kantong</div>
                </div>
                <div className="bg-white p-3 rounded-xl border border-blue-100 shadow-sm">
                  ...
                  <div className="text-[10px] font-bold text-slate-500 mt-1 bg-slate-50 px-2 py-1 rounded inline-block border border-slate-100">{formatNumber(kalkulasiOtomatis.hasilMika)} Mika / {formatNumber(kalkulasiOtomatis.hasilPorsi)} Porsi</div>
                </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border flex flex-col overflow-hidden shadow-sm">
          <div className="p-4 bg-slate-50 border-b flex items-center justify-between">
            ...
            {isHQ && (
              <select value={activeBranchFilter} onChange={e => setActiveBranchFilter(e.target.value)} className="text-[10px] font-black uppercase bg-white border rounded-lg px-2 py-1 outline-none text-slate-600 cursor-pointer shadow-sm"><option value="SEMUA_CABANG">🌍 NASIONAL</option>{daftarCabangId.map(b => <option key={b} value={b}>{b}</option>)}</select>
            )}
          </div>
                ...
                {historyProduksi.map(log => {
                  ...
                  return (
                    <tr key={log.id} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap"><div className="uppercase text-slate-700 font-black">{emp?.name || log.pic_id || 'UNKNOWN'}</div><div className="text-[9px] font-black text-indigo-400 mt-0.5 tracking-wider uppercase">LOK: {log.branch_id}</div></td>
                      <td className="px-4 py-3 text-center whitespace-nowrap"><div className="inline-flex items-center justify-center bg-emerald-100 text-emerald-800 px-3 py-1 rounded-lg text-sm font-black border border-emerald-200 shadow-sm">{log.total_adukan}</div></td>
                      ...
                      <td className="px-4 py-3 text-center whitespace-nowrap">
                        <div className="flex items-center justify-center gap-1.5">
                          <button type="button" onClick={() => {
                            ...
                            triggerPrint('NOTA_DOTMATRIX', { title: 'LAPORAN HASIL PRODUKSI PABRIK', id: log.id, date: formatDate(log.date), branch_name: log.branch_id || currentBranch, admin_name: user?.name || 'SISTEM', customer_name: emp?.name || 'TIM PRODUKSI', items: [{ name: rahasiaJson, qty: 1, subtotal: 0 }] });
