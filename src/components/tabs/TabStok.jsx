...
export default function TabOrders({ orders, payments, sendToSheet, setPrintData, role }) {
  ...
  const [form, setForm] = useState({
      date: todayStr,
      source: 'GOFOOD',
      customerName: 'Customer UAT',
      itemName: 'DIMSUM FROZEN',
      qty: '',
      price: '2500',
      paidAmount: '0',
  ...
  const handleSubmit = (e) => {
      ...
      const orderId = generateId('ORD', form.date);
      const settlementStatus = form.paymentMethod === 'MARKETPLACE' ? 'PENDING' : 'SETTLED';
      ...
      <div className="bg-white rounded-2xl border shadow-sm p-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-600 uppercase">Sumber Order</label>
                  <select value={form.source} onChange={e=>setForm({...form, source: e.target.value})} className="w-full p-3 bg-slate-50 border rounded-xl font-bold text-sm">
                      <option value="OFFLINE">Offline / Toko</option>
                      <option value="GOFOOD">GoFood (Fee 20%)</option>
                      <option value="GRABFOOD">GrabFood (Fee 20%)</option>
                      <option value="SHOPEEFOOD">ShopeeFood (Fee 22%)</option>
                      <option value="TIKTOK">TikTok (Fee 5%)</option>
                  </select>
              </div>
              <div className="space-y-1.5 md:col-span-2"><label className="text-[10px] font-bold text-slate-600 uppercase">Nama Pelanggan / Catatan</label><input type="text" required value={form.customerName} onChange={e=>setForm({...form, customerName: e.target.value})} className="w-full p-3 bg-slate-50 border rounded-xl font-bold text-sm" /></div>
              
              <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-600 uppercase">Barang</label>
                  <select value={form.itemName} onChange={e=>setForm({...form, itemName: e.target.value})} className="w-full p-3 bg-slate-50 border rounded-xl font-bold text-sm">
                      <option value="DIMSUM FROZEN">Dimsum Frozen</option>
                      <option value="DIMSUM MATANG">Dimsum Matang</option>
                  </select>
              </div>
              <div className="space-y-1.5"><label className="text-[10px] font-bold text-blue-600 uppercase">Qty (Pcs Terjual)</label><input type="number" required placeholder="0" value={form.qty} onChange={e=>setForm({...form, qty: e.target.value})} className="w-full p-3 bg-blue-50 border border-blue-200 rounded-xl font-black text-blue-700" /></div>
              <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-600 uppercase">Harga Jual Satuan (Rp)</label><input type="number" required value={form.price} onChange={e=>setForm({...form, price: e.target.value})} className="w-full p-3 bg-slate-50 border rounded-xl font-black text-slate-700" /></div>
              
              <div className="space-y-1.5"><label className="text-[10px] font-bold text-emerald-600 uppercase">Uang Diterima Hari Ini</label><input type="number" required value={form.paidAmount} onChange={e=>setForm({...form, paidAmount: e.target.value})} className="w-full p-3 bg-emerald-50 border border-emerald-200 rounded-xl font-black text-emerald-700" /></div>
              <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-600 uppercase">Metode Pembayaran</label>
                  <select value={form.paymentMethod} onChange={e=>setForm({...form, paymentMethod: e.target.value})} className="w-full p-3 bg-slate-50 border rounded-xl font-bold text-sm">
                      ...
                      <option value="MARKETPLACE">Tertahan di Marketplace</option>
                  </select>
              </div>
              ...
              <div className="md:col-span-3 bg-blue-50 p-4 rounded-xl border border-blue-200 flex justify-between items-center mt-2 shadow-inner">
                  ...
                  <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-black px-8 py-3.5 rounded-xl shadow-md transition tracking-wide uppercase text-xs flex items-center gap-2">
                      <CheckCircle size={16}/> Submit Order & Jurnal
                  </button>
              </div>
          </form>
      </div>
              ...
              <table className="w-full text-sm text-left">
                  <thead className="bg-slate-50 text-[10px] text-slate-500 uppercase">
                      <tr>
                          ...
                          <th className="px-4 py-3">Sumber & Pelanggan</th>
                      ...
                      {listOrders.map(o => (
                          <tr key={o.id} className="hover:bg-slate-50 transition">
                              ...
                              <td className="px-4 py-3">
                                  <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase mb-1 inline-block ${o.source === 'OFFLINE' ? 'bg-slate-200 text-slate-700' : 'bg-blue-100 text-blue-700'}`}>{o.source}</span>
                                  <div className="font-bold text-slate-800 text-xs truncate max-w-[150px]">{o.customer_name}</div>
                              </td>
                              <td className="px-4 py-3 text-center">
                                  <div className="font-black text-blue-600">{o.qty} Pcs</div>
                                  <div className="text-[9px] font-bold text-slate-500 uppercase">{o.itemName}</div>
                              </td>
                              <td className="px-4 py-3 text-right font-bold text-slate-700">{formatRp(o.total)}</td>
                              <td className="px-4 py-3 text-center">
                                  {Number(o.fee_amount) > 0 ? (
                                      <div className="text-red-600 font-bold text-xs">-{formatRp(o.fee_amount)}</div>
