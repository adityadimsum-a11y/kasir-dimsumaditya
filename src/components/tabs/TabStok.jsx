      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-5 flex flex-col bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden border-t-4 border-t-amber-500">
          <div className="p-6 border-b bg-amber-50/50 shrink-0">
             <h4 className="font-black text-slate-800 uppercase text-xs tracking-widest flex items-center gap-2"><Factory size={16} className="text-amber-600"/> Laporan Hasil Produksi</h4>
          </div>
          ...
          <form onSubmit={handleSubmitProduction} className="p-6 space-y-5 bg-white">
            ...
            <div>
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-1.5">Variant Produk Jadi</label>
            ...
            {/* LANGKAH 1 */}
            <div className="bg-amber-50/50 p-5 rounded-2xl border border-amber-200/70 shadow-inner relative">
              ...
              <label className="text-[10px] font-black text-amber-800 uppercase tracking-widest block mb-2 text-center mt-1">TOTAL ADUKAN HARI INI</label>
              <input type="number" min="1" required value={form.adukanQty} onChange={e=>handleAdukanChange(e.target.value)} className="w-full py-3.5 border-2 border-amber-300 rounded-xl text-3xl font-black text-amber-900 bg-white outline-none text-center focus:border-amber-500 transition-colors" placeholder="Cth: 21" />
            </div>

            {/* 🔥 LANGKAH 2: ADJUSTED TAMPILAN MAKIN KLOP & SIMETRIS */}
            <div className="bg-emerald-50/40 p-5 rounded-2xl border border-emerald-200/80 shadow-sm relative animate-in slide-in-from-bottom-2 duration-300">
              <div className="absolute -top-3 left-4 bg-emerald-600 text-white text-[8px] font-black px-2 py-1 rounded uppercase tracking-widest flex items-center gap-1 shadow-sm"><PackageCheck size={10}/> LANGKAH 2</div>
              <label className="text-[10px] font-black text-emerald-800 uppercase tracking-widest block mb-3 text-center mt-1">HASIL KEMASAN FISIK NYATA</label>
              
              {/* Box Input Wrapper - Dibuat Sejajar, Tinggi Sama Rata */}
              <div className="flex items-stretch gap-2">
                <input 
                  ...
                  className="flex-1 p-3.5 border-2 border-emerald-300 rounded-xl text-2xl font-black text-emerald-900 bg-white outline-none text-center focus:border-emerald-500 shadow-inner" 
                  placeholder="0" 
                />
                <select 
                  value={form.actualUnit} 
                  onChange={e=>setForm({...form, actualUnit: e.target.value})} 
                  className="w-32 px-3 bg-emerald-800 text-white rounded-xl text-xs font-black uppercase outline-none cursor-pointer border border-emerald-700 shadow-md text-center hover:bg-emerald-900 transition-colors"
                >
              ...
              {/* Live Preview Ringkas & Bersih di Bagian Bawah Input */}
              {form.actualInput && (
                <div className="mt-4 pt-3.5 border-t border-emerald-200/80 grid grid-cols-3 gap-2 animate-in fade-in duration-200">
                  <div className="bg-white/90 border border-emerald-200 rounded-xl p-2 text-center shadow-sm">
                  ...
                  <div className="bg-white/90 border border-emerald-200 rounded-xl p-2 text-center shadow-sm">
                    <div className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">SETARA MIKA</div>
                    <div className="text-xs font-black text-blue-700">{kalkulasi.previewMika} M</div>
                  </div>
                  <div className="bg-white/90 border border-emerald-200 rounded-xl p-2 text-center shadow-sm">
                    <div className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">SETARA PORSI</div>
                    <div className="text-xs font-black text-amber-700">{formatNumber(kalkulasi.previewPorsi)} P</div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <input type="text" value={form.notes} onChange={e=>setForm({...form, notes: e.target.value})} className="w-full p-3 border border-slate-200 rounded-xl text-xs font-bold uppercase outline-none bg-slate-50 focus:border-amber-400" placeholder="Adonan sisa kerokan panci..." />
            </div>

            <button type="submit" className="w-full bg-slate-900 text-white font-black py-4 rounded-2xl text-xs uppercase tracking-widest shadow-xl hover:bg-slate-800 transition-transform active:scale-95 flex items-center justify-center gap-2 mt-2">
            ...
            <table className="w-full text-sm text-left border-collapse">
              <thead className="bg-white text-[10px] uppercase text-slate-400 border-b border-slate-100 sticky top-0 shadow-sm">
                <tr>
                  ...
                  <th className="px-5 py-4 text-center font-black">Ayam Dipotong</th>
                  <th className="px-5 py-4 text-right font-black">Hasil Aktual (POS)</th>
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-emerald-50/30 transition-colors group">
                        ...
                        <td className="px-5 py-4 text-right whitespace-nowrap">
                          ...
                          <div className="text-[8px] font-black text-slate-400 mt-1 uppercase tracking-widest">{formatNumber(Math.floor((p.actual_yield || p.qty)/50))} Mika | {formatNumber(Math.floor((p.actual_yield || p.qty)/4))} Porsi</div>
                        </td>
