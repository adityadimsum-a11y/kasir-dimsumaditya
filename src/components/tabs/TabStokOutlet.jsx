...
import { Package, Download, Plus, CheckCircle, Layers, Truck } from 'lucide-react';
...
export default function TabStokOutlet({ distributionOrders, stockMovements, sendToSheet, showToast, user }) {
  ...
  const outletStock = useMemo(() => {
      ...
      (stockMovements || []).forEach(m => {
          ...
          if (['PRODUCTION_RESULT', 'SALE_RETURN', 'INVENTORY_IN', 'DISTRIBUTION_RECEIPT'].includes(m.movement_type) || (m.to_location && m.to_location.includes('FREEZER'))) {
              stock[item] += Number(m.qty) || 0;
          } else if (['SALE', 'WASTE', 'INVENTORY_OUT', 'DISTRIBUTION_USAGE'].includes(m.movement_type) || (m.from_location && m.from_location.includes('FREEZER'))) {
              stock[item] -= Number(m.qty) || 0;
          }
      });
      return stock;
  ...
  const handleTerimaBarang = async (doItem) => {
      ...
      if (successMove) {
          ...
          if (showToast) showToast('✅ Stok berhasil dimasukkan ke freezer toko.', 'success');
      }
  };
  ...
  const handleSimpanMenuLokal = async (e) => {
      ...
      const payloadStock = {
          ...
          branch_id: currentBranch,
          reference_id: generateId('LCL', new Date())
      };

      const success = await sendToSheet('insert', payloadStock, 'stock_movements');
      if (success) {
          if (showToast) showToast(`✅ Menu ${formMenuLokal.item_name} berhasil didaftarkan!`, 'success');
    ...
    <div className="space-y-6 animate-in fade-in pb-10">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="bg-slate-900 px-6 py-4 flex items-center justify-between">
              <h3 className="font-black text-white text-sm tracking-wide uppercase flex items-center gap-2"><Truck size={18} className="text-orange-400"/> Konfirmasi Surat Jalan Masuk</h3>
          ...
          <div className="p-4 divide-y">
              {incomingDOs.length === 0 ? (
                  <div className="text-center py-6 text-xs text-slate-400 font-bold">Tidak ada pengiriman logistik aktif menuju toko Anda.</div>
              ) : (
                  incomingDOs.map(d => (
                      <div key={d.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-3 first:pt-0 last:pb-0 gap-4">
                          <div>
                              <div className="text-xs font-black text-slate-800 uppercase">{d.item_name || 'DIMSUM FROZEN'} ➔ <span className="text-orange-600">{Number(d.qty).toLocaleString('id-ID')} PCS</span></div>
                              <div className="text-[10px] text-slate-500 font-bold mt-0.5">ID Dokumen: {d.id} • Tgl: {formatDate(d.date)}</div>
                          </div>
                          <button onClick={() => handleTerimaBarang(d)} className="bg-orange-600 hover:bg-orange-700 text-white text-xs font-black px-4 py-2 rounded-xl uppercase tracking-wide shadow-md transition flex items-center gap-1.5 w-full sm:w-auto justify-center"><Download size={14}/> Terima & Masuk Freezer</button>
                      </div>
                  ))
              )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white rounded-2xl border shadow-sm p-6 flex flex-col">
              <h3 className="font-black text-slate-800 text-sm tracking-wide uppercase flex items-center gap-2 mb-4"><Layers size={18} className="text-slate-500"/> Sisa Stok Freezer Toko</h3>
              <div className="overflow-x-auto flex-1">
                  <table className="w-full text-sm text-left">
                      <thead className="bg-slate-50 text-[10px] text-slate-500 uppercase">
                          <tr><th className="px-4 py-3">Nama Varian</th><th className="px-4 py-3 text-center">Tipe</th><th className="px-4 py-3 text-right">Volume Stok</th></tr>
                      </thead>
                          ...
                          {Object.keys(outletStock).map(key => {
                              ...
                              return (
                                  <tr key={key} className="hover:bg-slate-50">
                                      ...
                                      <td className="px-4 py-3 text-center"><span className="bg-purple-100 text-purple-800 px-2 py-0.5 rounded text-[9px]">Menu Lokal</span></td>
          ...
          <div className="lg:col-span-1 bg-white rounded-2xl border shadow-sm p-6 border-t-4 border-t-orange-500 h-max">
              <div className="flex items-center gap-3 mb-4 border-b pb-3"><div className="bg-orange-100 text-orange-700 p-2 rounded-lg"><Plus size={18}/></div><div><h3 className="font-black text-slate-800 text-xs uppercase tracking-wide">Registrasi Menu Lokal</h3></div></div>
              <form onSubmit={handleSimpanMenuLokal} className="space-y-4">
                  <div className="space-y-1">
                      ...
                      <input type="text" required value={formMenuLokal.item_name} onChange={e=>setFormMenuLokal({...formMenuLokal, item_name: e.target.value.toUpperCase()})} placeholder="Cth: ES TEH MANIS" className="w-full p-2.5 bg-slate-50 border rounded-xl font-bold text-xs uppercase" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                          <label className="text-[10px] font-bold text-slate-600 uppercase">Stok Awal</label>
                          <input type="number" required min="0" value={formMenuLokal.initial_qty} onChange={e=>setFormMenuLokal({...formMenuLokal, initial_qty: e.target.value})} className="w-full p-2.5 bg-slate-50 border rounded-xl font-bold text-xs" />
                      </div>
                      <div className="space-y-1">
                          ...
                          <div className="relative">
                              ...
                              <input type="text" required value={formMenuLokal.selling_price ? Number(formMenuLokal.selling_price).toLocaleString('id-ID') : ''} onChange={e => { const raw = e.target.value.replace(/\D/g, ''); setFormMenuLokal(prev=>({...prev, selling_price: raw})); }} className="w-full pl-8 pr-2 py-2.5 bg-orange-50 border border-orange-200 rounded-xl font-black text-orange-700 text-xs outline-none" />
                          </div>
                      </div>
                  </div>
                  <button type="submit" className="w-full bg-slate-900 hover:bg-slate-800 text-white font-black py-3.5 rounded-xl uppercase tracking-wider text-[10px] mt-2 shadow-md">Daftarkan & Injeksi Stok</button>
              </form>
          </div>
      </div>
    </div>
