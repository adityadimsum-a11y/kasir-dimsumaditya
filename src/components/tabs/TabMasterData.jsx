...
import { 
  ...
  CheckCircle2, User, Phone, MapPin, List
} from 'lucide-react';
...
export default function TabMasterData({ 
  ...
  masterRawMaterials = [], master_raw_materials, // 🔥 INI KONEKSI DATA ITEM BIAYA
  ...
  // 🔥 STATE FORM MASTER ITEM & BIAYA BARU!
  const [formItem, setFormItem] = useState({
    id: '', item_name: '', category: 'BAHAN BAKU', unit: '', default_price: ''
  });
  ...
  // 🔥 ACTIONS: SUBMIT MASTER ITEM & BIAYA (BARU)
  const handleSubmitItem = async (e) => {
    ...
    const payload = {
      ...
      unit: formItem.unit.toUpperCase(), default_price: Number(formItem.default_price || 0)
    };
    const isSuccess = await sendToSheet(isEditingItem ? 'update' : 'insert', payload, 'master_raw_materials');
    if (isSuccess) {
      showToast(isEditingItem ? 'Data Item Biaya diperbarui!' : 'Item Biaya Baru terdaftar!', 'success');
    ...
    <div className="space-y-6 pb-10 text-slate-800 animate-in fade-in duration-300">
      ...
      {activeTab === 'MENU' && (
         <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 animate-in fade-in">
           {/* Form Menu - Identik seperti sebelumnya */}
           <div className="xl:col-span-4">
             <div className="bg-white rounded-3xl border shadow-sm p-6 border-blue-200">
               <h3 className="font-black text-slate-800 uppercase tracking-widest text-sm mb-5 flex items-center gap-2"><Box size={18} className="text-blue-600"/> {isEditing ? 'Edit Menu' : 'Tambah Menu'}</h3>
      ...
      {activeTab === 'SUPPLIER' && (
         <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 animate-in fade-in">
           {/* Form Supplier - Identik seperti sebelumnya */}
           <div className="xl:col-span-4 bg-white rounded-3xl border shadow-sm p-6 border-emerald-200">
              <h3 className="font-black text-slate-800 uppercase tracking-widest text-sm mb-5 flex items-center gap-2"><Truck size={18} className="text-emerald-600"/> Tambah Supplier</h3>
           ...
           <div className="xl:col-span-8 bg-white rounded-3xl border shadow-sm overflow-hidden custom-scrollbar min-h-[50vh] overflow-x-auto">
             ...
             <tbody className="divide-y text-xs font-bold">
               {activeSuppliers.map(s => <tr key={s.id}><td className="px-5 py-4 uppercase">{s.supplier_name}</td><td className="px-5 py-4">{s.phone}</td><td className="px-5 py-4 text-center"><button onClick={() => {setFormSpl(s); setIsEditingSpl(true);}} className="p-2 text-amber-500"><Edit2 size={14}/></button></td></tr>)}
             </tbody></table>
           </div>
         </div>
      )}

      {/* 🔥 TAB SAKTI BARU: MASTER ITEM & BIAYA */}
      {activeTab === 'ITEM_BIAYA' && (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 animate-in fade-in">
          {/* SISI KIRI: INPUT MASTER ITEM */}
          <div className="xl:col-span-4">
            <div className={`bg-white rounded-3xl border shadow-sm p-6 md:p-8 transition-all ${isEditingItem ? 'border-amber-300' : 'border-rose-200'}`}>
              ...
              <form onSubmit={handleSubmitItem} className="space-y-4">
                ...
                <button type="submit" className="w-full bg-rose-600 text-white font-black py-4 rounded-xl text-xs uppercase tracking-widest shadow-md hover:bg-rose-700 active:scale-95 transition-all mt-2">
                   Simpan Master Item
                </button>
              </form>
            </div>
                ...
                <tbody className="divide-y text-xs font-bold">
                  {activeRawMaterials.length === 0 ? (
                    ...
                    activeRawMaterials.map(m => (
                      <tr key={m.id} className="hover:bg-rose-50/10">
                        <td className="px-5 py-4"><div className="text-sm font-black text-slate-800 uppercase">{m.item_name}</div><div className="text-[9px] text-slate-500 mt-0.5 tracking-widest uppercase">SATUAN: {m.unit}</div></td>
                        <td className="px-5 py-4 uppercase"><span className="bg-slate-100 border text-slate-600 px-2.5 py-1 rounded-md text-[9px] font-black">{m.category}</span></td>
                        <td className="px-5 py-4 text-right uppercase text-rose-600 font-black text-sm">{formatRupiah(m.default_price)}</td>
                        <td className="px-5 py-4 text-center">
                          <button onClick={() => { setFormItem({ id: m.id, item_name: m.item_name, category: m.category, unit: m.unit, default_price: String(m.default_price) }); setIsEditingItem(true); }} className="p-2 text-slate-400 hover:text-amber-500"><Edit2 size={14}/></button>
                          <button onClick={async () => { if(window.confirm("Hapus item ini dari kamus pabrik?")) sendToSheet('delete', {id: m.id}, 'master_raw_materials'); }} className="p-2 text-slate-400 hover:text-rose-500"><Trash2 size={14}/></button>
                        </td>
                      </tr>
                    ))
                  )}
        ...
        <div className="bg-[#151a25] rounded-3xl border p-6 shadow-2xl overflow-hidden text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-[#1c2331] p-5 rounded-xl border border-slate-700/50">
              ...
              <div className="text-xl font-black text-white">{rules.kgPerKantong} KG = <span className="text-amber-400">1 Kantong</span></div>
            </div>
            <div className="bg-[#1c2331] p-5 rounded-xl border border-slate-700/50">
              <div className="text-[9px] font-black text-emerald-400 uppercase tracking-widest mb-3">Rule #2: Resep Base</div>
              <div className="text-xl font-black text-white">{rules.kgPerAdukan} KG = <span className="text-amber-400">1 Adukan</span></div>
            </div>
          </div>
        </div>
      )}
