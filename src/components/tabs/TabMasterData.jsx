...
import { 
  ...
  Plus, Edit2, Trash2, CheckCircle2, AlertTriangle, Save, X 
} from 'lucide-react';
...
export default function TabMasterData({ 
  ...
  // --- SINKRONISASI DATABASE (MENCEGAH NAMA ARRAY KOSONG) ---
  ...
  const handleSubmitMenu = async (e) => {
    ...
    const payload = {
      ...
      status_active: true,
      min_order: Number(formMenu.min_order || 1),
      penalty_price: Number(formMenu.penalty_price || 0)
    };
    ...
    if (isSuccess) {
      showToast(isEditing ? 'Data Menu berhasil diperbarui!' : 'Menu Baru berhasil ditambah & sinkron ke Kasir POS!', 'success');
  ...
  const handleEditMenu = (menu) => {
    setFormMenu({
      ...
      selling_price: String(menu.selling_price || 0),
      default_hpp: String(menu.default_hpp || 0),
      min_order: String(menu.min_order || 1),
      penalty_price: String(menu.penalty_price || 0)
    });
    setIsEditing(true);
  ...
  const handleDeleteMenu = async (id) => {
    if (window.confirm("PERINGATAN! Anda yakin ingin menghapus menu ini dari database kasir?")) {
      const isSuccess = await sendToSheet('delete', { id }, 'master_products');
      if (isSuccess) {
        showToast('Menu berhasil dihapus dari sistem POS.', 'success');
      }
    }
  };
  ...
  return (
    <div className="space-y-6 pb-10 text-slate-800 animate-in fade-in duration-300">
            ...
            <div className={`bg-white rounded-3xl border shadow-sm p-6 md:p-8 transition-all duration-300 ${isEditing ? 'border-amber-300 shadow-amber-500/10' : 'border-blue-200'}`}>
              ...
              <form onSubmit={handleSubmitMenu} className="space-y-5">
                ...
                <div className="bg-amber-50/50 border border-amber-200 p-5 rounded-2xl shadow-inner">
                  ...
                  <div className="text-[8px] font-black text-amber-600/70 mt-3 uppercase tracking-wider leading-relaxed">
                    Jika pelanggan beli di bawah {formMenu.min_order} Pcs, sistem POS akan otomatis mengunci harga di {formatRupiah(formMenu.penalty_price)}/Pcs.
                  </div>
                </div>
                ...
                <button type="submit" className={`w-full text-white font-black py-4.5 rounded-2xl text-xs uppercase tracking-widest shadow-xl transition-transform active:scale-95 flex items-center justify-center gap-2 mt-2 ${isEditing ? 'bg-amber-500 hover:bg-amber-600 shadow-amber-500/30' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-600/30'}`}>
              ...
              <table className="w-full text-sm text-left border-collapse">
                <thead className="bg-white border-b text-[10px] uppercase text-slate-400 sticky top-0 shadow-sm">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-black text-center">Aturan Min. Order</th>
                    <th className="px-6 py-4 font-black text-center">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50 text-xs font-bold">
                  {activeProducts.length === 0 ? (
                    <tr><td colSpan="5" className="text-center py-20 text-slate-400 uppercase tracking-widest font-black bg-slate-50/50">Belum ada daftar menu di dalam database.</td></tr>
                  ) : (
                      ...
                      <tr key={p.id} className="hover:bg-blue-50/30 transition-colors group">
                        ...
                        <td className="px-6 py-4 text-center whitespace-nowrap opacity-50 group-hover:opacity-100 transition-opacity">
                          <div className="flex items-center justify-center gap-1.5">
                            <button type="button" onClick={() => handleEditMenu(p)} className="p-2.5 text-slate-500 bg-white border border-slate-200 shadow-sm hover:bg-amber-50 hover:text-amber-600 rounded-xl transition-colors" title="Edit Menu"><Edit2 size={16}/></button>
                            <button type="button" onClick={() => handleDeleteMenu(p.id)} className="p-2.5 text-slate-500 bg-white border border-slate-200 shadow-sm hover:bg-rose-50 hover:text-rose-600 rounded-xl transition-colors" title="Hapus Menu"><Trash2 size={16}/></button>
                          </div>
                        </td>
                      </tr>
                    ))
      ...
      {/* Sisa tab rules, raw material, dll bisa ditambahkan nanti di sini sesuai kebutuhan */}
      {activeTab !== 'MENU' && (
