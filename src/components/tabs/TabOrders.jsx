...
export default function TabOrders({ orders, payments, sendToSheet, setPrintData, requestDelete, role }) {
  ...
  const QUICK_TAGS = ["Tanpa Udang", "Tanpa Telur Puyuh", "Tanpa Udang & Puyuh", "Original Semua", "Siomay Semua", "Frozen", "Packing Pisah", "Tanpa Bawang", "Tanpa Saus"];
  ...
  const updatePaymentItem = (index, field, value) => {
      const newList = [...paymentList]; newList[index] = { ...newList[index], [field]: value }; setPaymentList(newList);
  };
  ...
  const resetForm = () => {
    ...
    setCart([{ category: defaultCat, qty: '', price: defaultPrice }]); setPaymentList([]);
  };
  
  const handleEdit = (item) => {
    const relatedItems = (orders||[]).filter(p => p.id === item.id);
    setDate(String(item.date).split('T')[0]); setCustomer(item.customer); setStatusProd(item.statusProduksi || 'Menunggu Produksi');
    
    let rawNotes = item.notes || ''; let extractedTags = [];
    if (rawNotes.includes('[TAGS:')) {
        const tagPart = rawNotes.match(/\[TAGS:(.*?)\]/);
        if (tagPart) { extractedTags = tagPart[1].split(', '); rawNotes = rawNotes.replace(tagPart[0], '').trim(); }
    }
  ...
  const handleSimpan = (e) => {
    ...
    const newOrdersArray = cart.filter(item => Number(item?.qty) > 0).map((item, index) => ({
        ...
        paymentMethod: index === 0 ? paymentDataString : '-', paidAmount: index === 0 ? totalDibayar : 0, 
        notes: finalNotes, isSpkPrinted: isEdit ? (orders.find(o => o.id === editId)?.isSpkPrinted || false) : false, 
        statusProduksi: statusProd, editCount: isEdit ? editCount + 1 : 0 
      }));
  ...
  const handlePrintSPK = (ord) => {
      ...
      if (!ord.isSpkPrinted) {
          ...
          sendToSheet('delete', { id: ord.id }, 'orders'); setTimeout(() => { sendToSheet('insert', rowsToUpdate, 'orders'); }, 300); 
      }
  };

  const handlePrintInvoice = (ord) => {
      // Gabungkan pembayaran awal (JSON) + cicilan dari TabPiutang agar tercetak di Invoice
      const cicilan = (payments || []).filter(p => p.orderId === ord.id);
      let basePayments = [];
      try { basePayments = JSON.parse(ord.paymentMethod); } catch(e) { if(ord.paidAmount > 0) basePayments = [{ method: ord.paymentMethod, amount: ord.paidAmount, date: ord.date }]; }
      const allPayments = [...basePayments, ...cicilan.map(c => ({ method: c.paymentMethod, amount: c.amount, date: c.date }))];
      
      setPrintData({ type: 'invoice', data: { ...ord, allPayments, sisaHutangAktual: ord.sisaTagihan } });
  };
  ...
  const handleStatusProduksiChange = (id, newStatus) => {
      ...
      sendToSheet('delete', { id }, 'orders'); setTimeout(() => { sendToSheet('insert', rowsToUpdate, 'orders'); }, 300); 
  };
  ...
  const displayOrders = useMemo(() => {
    ...
    filtered.forEach(p => {
        if(!p?.id) return;
        if(!groups[p.id]) groups[p.id] = { ...p, items: [], totalAll: 0, paidAmount: p.paidAmount, paymentMethod: p.paymentMethod, isSpkPrinted: p.isSpkPrinted === true || p.isSpkPrinted === 'true' || p.isSpkPrinted === 'TRUE', statusProduksi: p.statusProduksi || 'Menunggu Produksi' };
    ...
    <div className="space-y-4 animate-in fade-in pb-10">
      ...
      {showForm && (
        <form onSubmit={handleSimpan} className="bg-white p-6 rounded-xl border border-red-200 shadow-sm grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="lg:col-span-2 mb-2 border-b border-slate-100 pb-2 flex justify-between items-center"><h4 className="font-bold text-red-800 text-sm flex gap-2"><ShoppingCart size={16}/> Form {isEdit ? 'Edit' : 'Input'} Draft Pesanan</h4><button type="button" onClick={resetForm} className="text-slate-400 hover:text-red-500"><X size={18}/></button></div>
          ...
          <div className="space-y-4">
              ...
              <div className="bg-slate-100 p-4 rounded-xl border border-slate-200">
                  <h4 className="font-bold text-sm text-slate-700 mb-2 flex items-center gap-2"><ChefHat size={16}/> Catatan Dapur / Request</h4>
                  <div className="flex flex-wrap gap-2 mb-3">{QUICK_TAGS.map(tag => (<button key={tag} type="button" onClick={() => handleTagToggle(tag)} className={`px-2 py-1 rounded-md text-[10px] font-bold transition border ${selectedTags.includes(tag) ? 'bg-slate-800 text-white border-slate-800 shadow-sm' : 'bg-white text-slate-600 border-slate-300 hover:bg-slate-200'}`}>{tag}</button>))}</div>
                  <input type="text" value={notes} onChange={e => setNotes(e.target.value)} className="w-full p-2 border rounded-lg text-xs" placeholder="Ketik manual tambahan catatan dapur..." />
              </div>
          </div>
          ...
          <div className="space-y-4 flex flex-col">
              ...
              <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 flex-1 flex flex-col">
                 <div className="flex justify-between items-center mb-3"><h4 className="font-bold text-sm text-emerald-900 flex items-center gap-2"><CreditCard size={16}/> Riwayat / Input Pembayaran</h4><button type="button" onClick={addPaymentRow} className="bg-white px-3 py-1 text-[10px] font-bold text-emerald-600 border border-emerald-300 rounded hover:bg-emerald-100 transition shadow-sm">+ Tambah Pembayaran</button></div>
                 
                 {paymentList.length === 0 ? (
                     <div className="text-xs text-slate-500 italic text-center py-6 bg-white rounded border border-dashed border-emerald-200 mb-4">Belum ada pembayaran.<br/>Disimpan sebagai Draft (Belum Bayar).</div>
                 ) : (
                     <div className="space-y-2 mb-4">
                         {paymentList.map((item, index) => (
                             <div key={index} className="flex gap-2 items-center bg-white p-2 rounded-lg border border-emerald-200 relative pr-8">
                                 <div className="w-5/12 space-y-1"><select value={item.method} onChange={e=>updatePaymentItem(index, 'method', e.target.value)} className="w-full p-1.5 border rounded text-[10px] uppercase font-bold"><option>Cash</option><option>Transfer Bank</option><option>QRIS</option></select></div>
                 ...
                 <div className="border-t border-emerald-200 pt-3 mt-auto">
                     ...
                     <div className="flex justify-between items-center bg-white p-2 rounded border border-emerald-200 shadow-sm"><span className="text-xs font-bold text-slate-500 uppercase">Status Pembayaran Auto:</span><span className={`px-3 py-1 rounded text-[10px] font-black ${autoStatusBayar === 'LUNAS' ? 'bg-emerald-600 text-white' : autoStatusBayar === 'PIUTANG' ? 'bg-red-600 text-white shadow-md' : autoStatusBayar === 'DP' ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-600'}`}>{autoStatusBayar}</span></div>
                 </div>
              </div>
          </div>
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {displayOrders.length === 0 ? <tr><td colSpan="7" className="text-center py-12 text-slate-400">Tidak ada order ditemukan.</td></tr> : displayOrders.map((ord) => {
              ...
              let displayNotes = ord.notes || ''; let hasTags = false;
              if (displayNotes.includes('[TAGS:')) { displayNotes = displayNotes.replace(/\[TAGS:(.*?)\]/, (match, tags) => { hasTags = true; return `<span class="bg-slate-800 text-white px-1.5 py-0.5 rounded text-[9px] font-bold mr-1 block w-max mb-1">${tags}</span>`; }); }

              let displayVia = ord.paymentMethod;
              try { const parsed = JSON.parse(ord.paymentMethod); displayVia = parsed.map(p => p.method).join(' + '); } catch(e) {}
              ...
              <tr key={ord.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3"><div className="font-bold text-slate-800 uppercase text-xs mb-1">{ord.customer}</div>{ord.notes && <div className="text-[10px] text-slate-600 italic leading-tight" dangerouslySetInnerHTML={{ __html: displayNotes }}></div>}</td>
                ...
                <td className="px-4 py-3 text-center">
                    <select value={ord.statusProduksi} onChange={(e) => handleStatusProduksiChange(ord.id, e.target.value)} className={`text-[10px] font-bold p-1 rounded border outline-none cursor-pointer shadow-sm ${ord.statusProduksi === 'Sudah Diambil' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : ord.statusProduksi === 'Ready Diambil' ? 'bg-blue-50 text-blue-700 border-blue-200' : ord.statusProduksi === 'Diproses' ? 'bg-purple-50 text-purple-700 border-purple-200' : 'bg-orange-50 text-orange-700 border-orange-200'}`}>
                        <option value="Menunggu Produksi">Menunggu Produksi</option><option value="Diproses">Diproses</option><option value="Ready Diambil">Ready Diambil</option><option value="Sudah Diambil">Sudah Diambil</option>
                    </select>
                </td>
                ...
                <td className="px-4 py-3 text-center">
                  <div className="flex justify-center gap-1.5">
                    <button onClick={() => handlePrintSPK(ord)} className={`p-2 rounded-lg border transition shadow-sm flex items-center justify-center relative ${ord.isSpkPrinted ? 'bg-emerald-50 border-emerald-200 text-emerald-600 hover:bg-emerald-100' : 'bg-slate-800 border-slate-900 text-white hover:bg-slate-700'}`} title={ord.isSpkPrinted ? "Cetak Ulang SPK Produksi" : "Cetak SPK Perintah Produksi"}><ChefHat size={16} /></button>
                    <button onClick={() => handlePrintInvoice({ ...ord, sisaTagihan: sisaHutang })} className="text-blue-600 bg-blue-50 p-2 rounded-lg border border-blue-200 hover:bg-blue-100 transition shadow-sm" title="Cetak Invoice Customer"><Printer size={16} /></button>
                    <button onClick={() => handleEdit(ord)} className="text-slate-600 bg-slate-100 px-2 py-1 rounded-lg font-bold text-[10px] border border-slate-200 hover:bg-slate-200 transition shadow-sm">EDIT</button>
                    <button onClick={() => requestDelete(ord.id)} className="text-red-500 bg-red-50 p-2 rounded-lg border border-red-200 hover:bg-red-100 transition shadow-sm" title="Hapus Data"><Trash2 size={16} /></button>
                  </div>
                </td>
              </tr>
            )})}
