...
export default function TabOrders({ 
  ...
  // Normalisasi Nama Cabang (PUSAT = TANGERANG_PUSAT)
  ...
  // --- ALGORITMA MONITORING STOK SILANG (SUDAH DIPERBAIKI) ---
  const stockMetrics = useMemo(() => {
    let totalMasukFreezer = 0; let totalKeluarFreezer = 0;
    let totalAyamMasukKg = 0; let totalAyamKeluarKg = 0;
    let karantinaPcs = 0; let listKarantina = [];
    ...
    realPurchases.filter(p => !p.isDeleted && p.category === 'BAHAN_BAKU' && (p.branch_id === currentBranch || p.branch_id === 'PUSAT')).forEach(p => {
       totalAyamMasukKg += Number(p.qty_kg || 0); 
    });

    realProd.filter(p => !p.isDeleted && (p.branch_id === currentBranch || p.branch_id === 'PUSAT')).forEach(p => {
...
// --- HANDLER CETAK TIKET PRODUKSI 3-PLY (ARSIPIASI & VALIDASI) ---
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.paymentMethod === 'DP' && Number(form.amountPaid) < 0) return alert("Nominal DP tidak valid!");
    ...
    if (form.deliveryMethod === 'DIRECT') {
      ...
      if (Number(form.qty) > stockMetrics.sisaAvailable) {
        ...
        const confirmPinjam = window.confirm(`⚠️ WARNING MEMINJAM STOK KARANTINA!\n\nStok Bebas sisa ${stockMetrics.sisaAvailable} Pcs.\nOrder ini akan meminjam jatah ${pinjam} Pcs milik antrean orang lain!\nTetap eksekusi & catat log peminjaman?`);
    ...
    const payload = {
      ...
      custom_request: form.customRequest.toUpperCase(),
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={`p-6 rounded-2xl border border-t-4 transition-all h-max shadow-sm ${isEditing ? 'bg-amber-50/50 border-t-amber-500 border-amber-200' : 'bg-white border-t-emerald-600'}`}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select value={form.channel} onChange={e=>setForm({...form, channel: e.target.value})} className="w-full p-2.5 border border-slate-300 rounded-xl text-xs font-black outline-none bg-white uppercase cursor-pointer">
                  <optgroup label="Offline Node">{SALES_CHANNELS.filter(c => c.group === 'OFFLINE').map(c => <option key={c.id} value={c.id}>{c.label}</option>)}</optgroup>
                  <optgroup label="Marketplace Engine">{SALES_CHANNELS.filter(c => c.group === 'MARKETPLACE').map(c => <option key={c.id} value={c.id}>{c.label}</option>)}</optgroup>
                  <optgroup label="Merchant Delivery">{SALES_CHANNELS.filter(c => c.group === 'MERCHANT').map(c => <option key={c.id} value={c.id}>{c.label}</option>)}</optgroup>
                </select>
              </div>
              <div>
            ...
            <div>
              <label className="text-[10px] font-black text-rose-600 uppercase tracking-widest block mb-1">⚠️ Request Khusus Produksi</label>
              <input type="text" required value={form.customRequest} onChange={e=>setForm({...form, customRequest: e.target.value})} className="w-full p-2.5 border-2 border-rose-200 focus:border-rose-400 rounded-xl text-xs font-black uppercase outline-none bg-rose-50/30 text-rose-800" placeholder="CONTOH: HANYA SIOMAY / TANPA UDANG" />
            </div>
            ...
            <div className="p-4 rounded-xl border bg-white border-slate-200">
              ...
              {form.paymentMethod === 'DP' && (
                <div className="mt-3 pt-3 border-t border-dashed">
                  <label className="text-[10px] font-black text-amber-700 uppercase tracking-widest block mb-1">Nominal Setoran Awal / DP (Rp)</label>
                  <input type="number" required min="0" value={form.amountPaid} onChange={e=>setForm({...form, amountPaid: e.target.value})} className="w-full p-2 border border-amber-300 rounded-lg text-right font-black text-amber-700 bg-amber-50/50" />
          ...
          <div className="overflow-x-auto flex-1 custom-scrollbar">
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 text-[10px] uppercase text-slate-400 border-b border-slate-200">
                <tr><th className="px-4 py-3">Invoice &amp; Tanggal</th><th className="px-4 py-3">Klien &amp; Volume</th><th className="px-4 py-3">Spesifikasi Request</th><th className="px-4 py-3">Fulfillment</th><th className="px-4 py-3 text-center">Aksi / Dok. Pabrik</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-100 text-xs font-bold">
                {realOrders.filter(o => !o.isDeleted).sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 50).map(log => {
                  ...
                  const isBelumBayar = sisaUtang === Number(log.total_amount);
                  return (
                    <tr key={log.id} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap"><div className="uppercase text-slate-700 font-black">{log.customer_name}</div><div className="text-[9px] font-black text-indigo-500 mt-0.5 uppercase">{log.sales_channel} • {formatNumber(log.qty)} PCS</div></td>
                      <td className="px-4 py-3"><div className="text-slate-700 uppercase font-black tracking-tight line-clamp-2 max-w-[180px] bg-slate-100 px-2 py-1 rounded text-[10px] text-rose-700 border border-slate-200">{log.custom_request || 'STANDAR MIX'}</div></td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        ...
                        <div className={`text-[9px] font-black mt-0.5 uppercase tracking-widest px-1.5 py-0.5 rounded inline-block ${sisaUtang > 0 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                          {sisaUtang > 0 ? (isBelumBayar ? 'BELUM BAYAR' : `DP (${formatRupiah(log.amount_paid)})`) : 'LUNAS'}
                        </div>
                      </td>
                      <td className="px-4 py-3 text-center whitespace-nowrap">
                        <div className="flex items-center justify-center gap-1.5">
                          <button type="button" onClick={() => handlePrintTiketProduksi(log)} className="p-1.5 bg-rose-600 text-white hover:bg-rose-700 rounded-lg shadow-sm flex items-center gap-1 text-[10px] font-black uppercase"><FileText size={12}/> Tiket Pabrik</button>
                          ...
                          <button type="button" onClick={() => { if(window.confirm("Void invoice ini?")) requestDelete(log.id); }} className="p-1.5 bg-rose-50 text-rose-600 hover:bg-rose-100 rounded-lg"><Trash2 size={12}/></button>
                        </div>
                      </td>
                    </tr>
                  );
            ...
            <div className="overflow-y-auto flex-1 custom-scrollbar">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-50 text-[10px] uppercase text-slate-500 border-b sticky top-0 shadow-sm z-10">
                  <tr><th className="px-4 py-3">Tanggal &amp; ID INV</th><th className="px-4 py-3">Nama Pembeli</th><th className="px-4 py-3">Spesifikasi Request</th><th className="px-4 py-3 text-center">Volume Booking</th><th className="px-4 py-3 text-center">Aksi</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-100 text-xs font-bold">
