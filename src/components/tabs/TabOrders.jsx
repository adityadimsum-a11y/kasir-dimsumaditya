...
export default function TabOrders({ 
  ...
  return (
    <div className="space-y-6 pb-10 text-slate-800 animate-in fade-in duration-300">
      ...
      {/* 🚀 BANNER MONITOR STOK TERATAS (GLASSMORPHISM) */}
      <div className="card-holo rounded-3xl p-6 shadow-xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-2 mb-5">
           ...
           <h3 className="text-slate-800 font-black uppercase tracking-widest text-sm">Dashboard Stok Aktual Freezer</h3>
        </div>
        ...
        <div className="flex overflow-x-auto gap-6 pb-4 custom-scrollbar">
          {activeMenus.map(m => {
            ...
            return (
              <div key={m.id} className={`flex flex-col min-w-[320px] shrink-0 rounded-2xl overflow-hidden border shadow-sm ${isMix ? 'border-orange-300/50 bg-orange-50/40' : 'border-white/50 bg-white/40'}`}>
                 <div className={`p-4 border-b ${isMix ? 'border-orange-200/50 bg-orange-100/30' : 'border-white/50 bg-white/50'}`}>
                   <h4 className={`text-[11px] font-black uppercase tracking-widest leading-tight ${isMix ? 'text-orange-700' : 'text-slate-700'}`}>{m.product_name}</h4>
                 </div>
                 
                 <div className="p-4 grid grid-cols-3 gap-3">
                    {/* Kotak PCS Dasar (Pasti Muncul) */}
                    <div className="flex flex-col items-center justify-center p-3 bg-white/50 rounded-xl border border-white/60 shadow-sm">
                       <div className="text-[9px] font-bold text-slate-500 uppercase mb-1">TOTAL PCS</div>
                       <div className={`text-xl font-black ${stok > 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{formatNumber(stok)}</div>
                    </div>
                    ...
                    {isMix ? (
                      <>
                        {/* Kotak MIKA Konversi */}
                        <div className="flex flex-col items-center justify-center p-3 bg-white/50 rounded-xl border border-white/60 shadow-sm">
                           <div className="text-[9px] font-bold text-blue-600 uppercase mb-1">MIKA (50)</div>
                           <div className="text-lg font-black text-slate-800">{formatNumber(Math.floor(stok / 50))}</div>
                        </div>
                        {/* Kotak PORSI Konversi */}
                        <div className="flex flex-col items-center justify-center p-3 bg-white/50 rounded-xl border border-white/60 shadow-sm">
                           <div className="text-[9px] font-bold text-amber-600 uppercase mb-1">PORSI (4)</div>
                           <div className="text-lg font-black text-slate-800">{formatNumber(Math.floor(stok / 4))}</div>
                        </div>
                      </>
                    ) : (
                      <div className="col-span-2 flex items-center justify-center p-3 bg-white/30 rounded-xl border border-white/40">
                         <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Produk Tanpa Konversi</span>
                      </div>
                    )}
                 </div>
              </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-5 flex flex-col h-[70vh] card-holo rounded-3xl overflow-hidden">
          <div className="p-5 border-b border-white/50 bg-white/40 shrink-0">
             <h4 className="font-black text-slate-800 uppercase text-xs tracking-widest flex items-center gap-2"><Box size={16} className="text-orange-500"/> Katalog Menu (Klik Untuk Jual)</h4>
          </div>
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-white/20">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {activeMenus.map(m => {
                const stok = stockMap[m.product_name] || 0;
                return (
                  <div key={m.id} onClick={() => handleAddMenu(m)} className={`border rounded-2xl p-4 cursor-pointer transition-all hover:scale-[1.02] flex flex-col justify-between relative overflow-hidden shadow-sm ${stok <= 0 ? 'bg-red-50/60 border-red-200 hover:border-red-400' : 'bg-white/50 border-white/60 hover:border-orange-400'}`}>
                    ...
                    <div>
                      ...
                      <span className="text-[8px] font-black uppercase tracking-wider text-slate-500 bg-white/80 border border-white/60 px-1.5 py-0.5 rounded">{m.category.replace('_', ' ')}</span>
                    </div>
                    <div className="mt-4 pt-3 border-t border-white/50">
                      <div className="font-black text-holo-gradient text-sm mb-1">{formatRupiah(m.selling_price)}</div>
                      ...
                      <div className={`mt-2 text-[9px] font-black uppercase px-2 py-1 rounded-md inline-block tracking-widest ${stok > 0 ? 'bg-emerald-50/80 text-emerald-700 border border-emerald-200' : 'bg-rose-100/80 text-rose-700 border border-rose-200'}`}>
        ...
        <div className="xl:col-span-7 flex flex-col card-holo rounded-3xl overflow-hidden border-t-4 border-t-orange-500">
          <div className="p-5 border-b border-white/50 bg-white/40 shrink-0">
             <h4 className="font-black text-slate-800 uppercase text-xs tracking-widest flex items-center gap-2"><ShoppingCart size={16} className="text-orange-600"/> Keranjang Kasir</h4>
          </div>

          <div className="flex-1 overflow-y-auto p-5 custom-scrollbar bg-white/20">
            {cart.length === 0 ? (
              ...
              <div className="space-y-3">
                {cartCalculated.items.map(item => (
                  <div key={item.id} className={`p-4 border rounded-2xl bg-white/60 backdrop-blur-sm shadow-sm relative transition-all ${item.isEceran ? 'border-amber-300' : 'border-white/80'}`}>
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <h5 className="font-black text-xs uppercase text-slate-800">{item.name}</h5>
                        {item.isEceran ? 
                          <span className="text-[8px] font-black bg-amber-100/80 text-amber-700 px-2 py-0.5 rounded border border-amber-200 uppercase tracking-widest mt-1 inline-block">Terkena Pinalti Harga Ecer (Min: {item.min_order})</span> : 
                          <span className="text-[8px] font-black bg-emerald-50/80 text-emerald-700 px-2 py-0.5 rounded border border-emerald-200 uppercase tracking-widest mt-1 inline-block">Lolos Harga Grosir</span>
                        }
                      </div>
                      <button onClick={() => removeCartItem(item.id)} className="text-slate-400 hover:text-rose-500 transition-colors"><Trash2 size={16}/></button>
                    </div>
                    <div className="flex gap-3">
                      <div className="w-24">
                        <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-1">Jumlah (Pcs)</label>
                        <input type="number" min="1" value={item.qty} onChange={(e) => updateCartQty(item.id, Number(e.target.value))} className="w-full p-2 border border-white/80 rounded-lg text-xs font-black text-center outline-none focus:border-orange-400 bg-white/50" />
                      </div>
                      <div className="flex-1">
                        <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-1">Catatan Tambahan Khusus Dapur</label>
                        <input type="text" value={item.note} onChange={(e) => updateCartNote(item.id, e.target.value)} className="w-full p-2 border border-white/80 rounded-lg text-xs font-bold uppercase outline-none focus:border-orange-400 bg-white/50" placeholder="Cth: Mix Hakau, dll..." />
                      </div>
                    </div>
                    <div className="flex justify-between items-end mt-3 pt-3 border-t border-white/60">
                      <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Harga: {formatRupiah(item.hargaBerlaku)}/Pcs</span>
                      <span className="text-sm font-black text-holo-gradient">{formatRupiah(item.subtotal)}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="p-5 border-t border-white/50 bg-white/50 shrink-0 shadow-[0_-10px_30px_rgba(0,0,0,0.03)] relative z-10 backdrop-blur-md">
            <form onSubmit={handleCheckout} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[9px] font-black text-slate-600 uppercase tracking-widest block mb-1">Nama Pembeli / Pelanggan</label>
                  <input type="text" required value={customerName} onChange={e=>setCustomerName(e.target.value)} className="w-full p-3 border border-white/80 rounded-xl text-xs font-black uppercase outline-none focus:border-orange-500 bg-white/60 focus:bg-white transition-colors shadow-sm" placeholder="Ketik nama pelanggan..." />
                </div>
                <div>
                  <label className="text-[9px] font-black text-slate-600 uppercase tracking-widest block mb-1">Sumber Jalur Pembeli</label>
                  <select value={orderSource} onChange={e=>setOrderSource(e.target.value)} className="w-full p-3 border border-white/80 rounded-xl text-xs font-black uppercase outline-none cursor-pointer bg-white/60 focus:border-orange-500 transition-colors shadow-sm">
              ...
              <div className="bg-white/40 p-1.5 rounded-2xl flex gap-1.5 border border-white/60 shadow-sm">
                <button type="button" onClick={() => setFulfillment('LANGSUNG')} className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-1.5 ${fulfillment === 'LANGSUNG' ? 'bg-white shadow-sm border border-white text-blue-600' : 'text-slate-500 hover:text-slate-800'}`}><ShoppingCart size={14}/> Diambil Langsung</button>
                <button type="button" onClick={() => setFulfillment('PRE_ORDER')} className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-1.5 ${fulfillment === 'PRE_ORDER' ? 'bg-white shadow-sm border border-white text-amber-600' : 'text-slate-500 hover:text-slate-800'}`}><Clock size={14}/> Pre-Order (PO Ditahan)</button>
              </div>

              {/* 🚀 KOTAK TOTAL DENGAN GEMBOK RAHASIA HPP (GLASS SULTAN) */}
              <div className="bg-white/50 backdrop-blur-sm text-slate-800 p-5 rounded-2xl shadow-sm relative overflow-hidden border border-white/80">
                <div className="absolute top-0 left-0 w-1.5 h-full bg-gradient-to-b from-red-500 to-amber-500"></div>
                <div className="flex justify-between items-end mb-2 pl-3">
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-600 block mb-1">Total Tagihan Nota</span>
                    <span className="text-[9px] font-bold text-slate-500">Total Kuantitas: {formatNumber(cartCalculated.totalQty)} Pcs</span>
                  </div>
                  <span className="text-3xl font-black tracking-tight text-holo-gradient">{formatRupiah(cartCalculated.totalTagihan)}</span>
                </div>

                {/* HANYA MUNCUL JIKA AKUN PUSAT / TANGERANG (BOS SULTAN) */}
                {currentBranch === 'TANGERANG_PUSAT' && (
                  <div className="mt-4 pt-3 border-t border-white/60 space-y-1.5 pl-3">
                    <div className="flex justify-between text-[9px] font-black uppercase tracking-widest text-slate-500">
                      <span>Perkiraan Modal Pabrik (HPP):</span>
                      <span className="text-red-500">{formatRupiah(cartCalculated.totalHpp)}</span>
                    </div>
                    <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-slate-800">
              ...
              <div className="p-4 border border-white/60 rounded-2xl bg-white/40 shadow-sm">
                <div className="flex justify-between items-center mb-3">
                  <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Cara Pembayaran</label>
                  <div className="flex gap-1 bg-white/50 border border-white/50 p-1 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                  <select value={paymentMethod} onChange={e=>setPaymentMethod(e.target.value)} className="w-full p-3 border border-white/80 rounded-xl text-[10px] font-black uppercase outline-none cursor-pointer bg-white/80 focus:border-orange-500">
                  ...
                  {paymentType === 'DP' && (
                    <div className="relative animate-in fade-in zoom-in-95 duration-200">
                      ...
                      <input type="text" required value={dpAmount ? Number(dpAmount).toLocaleString('id-ID') : ''} onChange={e=>setDpAmount(e.target.value.replace(/\D/g, ''))} className="w-full pl-9 pr-3 py-3 border-2 border-blue-200/50 bg-blue-50/50 rounded-xl font-black text-sm text-blue-700 outline-none focus:border-blue-400" placeholder="0" />
                    </div>
                  )}
                </div>
              </div>

              <button type="submit" disabled={cart.length === 0} className="w-full btn-holo py-4.5 rounded-2xl text-xs uppercase tracking-widest shadow-xl flex justify-center items-center gap-2 disabled:opacity-50 disabled:active:scale-100">
      ...
      <div className="card-holo rounded-3xl flex flex-col overflow-hidden mt-2">
        <div className="p-5 bg-white/40 border-b border-white/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h4 className="font-black text-xs uppercase text-slate-800 tracking-widest flex items-center gap-2"><FileText size={16} className="text-orange-500"/> Riwayat Jurnal Transaksi POS</h4>
          ...
          <div className="flex items-center gap-2 bg-white/60 border border-white/80 px-3 py-2 rounded-xl shadow-sm">
            <Calendar size={14} className="text-orange-500 ml-0.5"/>
        ...
        <div className="overflow-x-auto flex-1 p-2 custom-scrollbar min-h-[40vh]">
          <table className="w-full text-sm text-left border-collapse">
            <thead className="bg-white/50 text-[10px] uppercase text-slate-500 border-b border-white/60">
              <tr>
            ...
            <tbody className="text-xs font-bold divide-y divide-white/40">
              {historyOrders.length === 0 ? (
                <tr>
                  <td colSpan="5" className="text-center py-16 text-slate-400 font-black uppercase tracking-widest bg-white/20">
                ...
                historyOrders.map(o => {
                  ...
                  if (sisaBelumDiambil <= 0) {
                    ...
                    badgeColor = 'bg-emerald-50/80 text-emerald-700 border-emerald-200/60';
                  } else if (qtyDelivered > 0 && sisaBelumDiambil > 0) {
                    statusBadge = '⏳ DIAMBIL SEBAGIAN';
                    badgeColor = 'bg-blue-50/80 text-blue-700 border-blue-200/60';
                  } else {
                    statusBadge = '⏳ PO DITAHAN FULL';
                    badgeColor = 'bg-amber-50/80 text-amber-700 border-amber-200/60';
                  }
                  ...
                  return (
                    <tr key={o.id} className="hover:bg-white/40 transition-colors group">
                      <td className="px-5 py-4 whitespace-nowrap">
                        ...
                        <div className="text-[9px] font-mono text-slate-500 mt-1">{o.id}</div>
                      </td>
                      <td className="px-5 py-4">
                        <div className="font-black text-orange-600 uppercase text-xs mb-1 tracking-wide">{o.customer_name}</div>
                        <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded border border-white/60 bg-white/80 text-slate-500 mb-2 inline-block tracking-widest">{String(o.sales_channel).replace('_', ' ')}</span>
                        <div className="space-y-1 mt-1">
                          {parsedItems.map((item, idx) => (
                             <div key={idx} className="text-[10px] text-slate-600 uppercase font-bold flex justify-between items-start border-b border-dashed border-white/50 pb-1">
                               <span>• {item.name} <span className="text-blue-500 font-black">(x{formatNumber(item.qty)})</span></span>
                               <span className="text-slate-500 shrink-0 ml-2">{formatRupiah(item.subtotal)}</span>
                             </div>
                          ))}
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4 text-right whitespace-nowrap min-w-[180px]">
                        ...
                        <div className="flex justify-between items-center text-[10px] font-bold text-emerald-600 mb-1 border-b border-white/50 pb-1">
                        ...
                        {isDP && (
                          <div className="flex justify-between items-center text-[11px] font-black text-red-600 uppercase"><span>Sisa Piutang:</span><span>{formatRupiah(sisaHutang)}</span></div>
                        )}
                        <div className="text-[8px] font-black text-slate-500 mt-1.5 uppercase text-right tracking-widest">Jalur: {String(o.payment_method || 'CASH').replace('_', ' ')}</div>
                      </td>
                      <td className="px-5 py-4 text-center whitespace-nowrap opacity-50 group-hover:opacity-100 transition-opacity">
                        <div className="flex flex-col items-center justify-center gap-1.5">
                          {sisaBelumDiambil > 0 && (
                            <button type="button" onClick={() => setDeliveryModal(o)} className="w-full py-2 px-3 text-[9px] font-black uppercase tracking-widest text-amber-700 bg-amber-100/80 border border-amber-300 shadow-sm hover:bg-amber-200 rounded-lg transition-colors flex items-center justify-center gap-1.5 mb-1"><PackageCheck size={14}/> Serahkan Barang</button>
                          )}
                          <div className="flex gap-1.5 w-full">
                            ...
                            })} className="flex-1 p-2 text-slate-500 bg-white/60 border border-white/80 shadow-sm hover:text-blue-600 hover:bg-white rounded-xl transition-colors flex justify-center" title="Cetak Ulang Tiket Nota"><Printer size={16}/></button>
                            
                            <button type="button" onClick={() => { if(window.confirm("PERINGATAN! Yakin ingin MENGHAPUS (Void) transaksi penjualan ini? Stok dan Omset akan ditarik kembali!")) requestDelete(o.id); }} className="flex-1 p-2 text-slate-500 bg-white/60 border border-white/80 shadow-sm hover:text-red-600 hover:bg-white rounded-xl transition-colors flex justify-center" title="Void Hapus Transaksi"><Trash2 size={16}/></button>
                          </div>
                        </div>
                      </td>
                    </tr>
      ...
      {/* 🚀 MODAL TRACKER PENGAMBILAN BARANG (AMBIL SEBAGIAN) - GLASSMORPHISM */}
      {deliveryModal && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex justify-center items-center p-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/50 w-full max-w-sm overflow-hidden flex flex-col">
               ...
               <form onSubmit={handleSubmitDelivery} className="p-6 space-y-5">
                 <div className="bg-amber-50/80 border border-amber-200/50 p-4 rounded-2xl shadow-inner text-center">
                   ...
                   <div className="text-xs font-black text-red-600 uppercase mt-2 pt-2 border-t border-amber-200/50 border-dashed">
                 ...
                 <div>
                   ...
                   <input type="number" min="1" max={sisaHakAmbil} required value={deliveryQty} onChange={e=>setDeliveryQty(e.target.value)} className="w-full p-4 border-2 border-emerald-200/50 rounded-xl text-lg font-black text-emerald-700 bg-emerald-50/50 text-center outline-none focus:bg-white focus:border-emerald-500 transition-colors" placeholder={`Maks: ${sisaHakAmbil}`} />
                 ...
                 <button type="submit" className="w-full text-white font-black py-4 rounded-xl text-xs uppercase tracking-widest shadow-xl btn-holo flex justify-center items-center gap-2">
