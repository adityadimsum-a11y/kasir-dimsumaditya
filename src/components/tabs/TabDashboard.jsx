...
import { Wallet, TrendingUp, Calendar, Printer, FileText, ArrowRightLeft, BellRing, Activity, ShoppingCart, Truck, Factory, AlertCircle, Clock } from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in pb-10">
      ...
      <div className="bg-white p-4 rounded-2xl border shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex gap-2 w-full md:w-auto">
              <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="p-2 text-sm font-bold border border-slate-300 rounded-xl w-full md:w-auto bg-slate-50 focus:ring-2 focus:ring-slate-800 outline-none transition" />
              <span className="text-slate-400 self-center font-bold text-xs uppercase">s/d</span>
              <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="p-2 text-sm font-bold border border-slate-300 rounded-xl w-full md:w-auto bg-slate-50 focus:ring-2 focus:ring-slate-800 outline-none transition" />
          </div>
      </div>

      {/* SUMMARY CARDS KECIL DI ATAS */}
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <StatCard title="Saldo Laci (Cash)" value={formatRp(dash.saldoCash)} icon={<Wallet size={64}/>} color="bg-emerald-50 border-emerald-200 text-emerald-900" subtitle="In: Cash" subValue={formatRp(dash.inCash)} />
      ...
      {/* AREA BAWAH: LAYOUT 3 KOLOM MEMANJANG (GRID 12) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4 items-start">
          
          {/* KOLOM KIRI: ACTIVITY FEED REALTIME (Lebar: 4/12) */}
                                  ...
                                  <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex-1 hover:border-slate-300 transition group">
                                      <div className="flex justify-between items-start mb-1">
                                          <div className="flex items-center gap-2">
                                              ...
                                              <span className="font-black text-[10px] uppercase text-slate-800">{f.title}</span>
                                          </div>
          ...
          {/* KOLOM TENGAH: FULL LIST PIUTANG & HUTANG (Lebar: 5/12) */}
          <div className="lg:col-span-5 flex flex-col gap-6 h-[700px] overflow-y-auto pr-2 custom-scrollbar">
              ...
              <div className="bg-white rounded-2xl border shadow-sm flex flex-col">
                  <div className="p-4 border-b bg-amber-50 flex items-center gap-3 sticky top-0 z-10 rounded-t-2xl">
                      <div className="bg-amber-100 p-1.5 rounded-lg text-amber-700"><Clock size={16}/></div>
                      <h3 className="font-bold text-sm text-amber-900 tracking-wide uppercase">Piutang Customer Berjalan</h3>
                  </div>
                  <div className="p-4 bg-slate-50/50 space-y-3">
                      {dash.piutangBerjalan.length === 0 ? (
                          <div className="text-center p-6 text-slate-400 text-[10px] italic">Tidak ada piutang customer aktif.</div>
                      ) : (
                          dash.piutangBerjalan.map((p, idx) => (
                              <div key={idx} className="bg-white border border-amber-200 rounded-xl p-3 shadow-sm hover:shadow-md transition relative overflow-hidden">
                                  ...
                                  <div className="flex justify-between items-center mb-2 pl-2">
                                      <span className="text-[9px] text-slate-500 font-bold">{formatDate(p.date)} • {p.time}</span>
                                      <span className="text-[9px] font-black bg-slate-100 text-slate-600 px-2 py-0.5 rounded">{p.id}</span>
                                  </div>
                                  <div className="font-black uppercase text-slate-800 text-sm mb-2 pl-2 truncate">{p.customer}</div>
                                  <div className="flex justify-between items-end pl-2">
                                      <div>
                                          <div className="text-[9px] text-slate-500 uppercase font-bold mb-0.5">Sisa Tagihan</div>
                                          <div className="font-black text-red-600 text-sm">{formatRp(p.sisaTagihan)}</div>
                                      </div>
                                      <div className="text-[9px] font-bold bg-amber-100 text-amber-800 px-2 py-1 rounded border border-amber-200">
                                          Status: {p.statusProduksi}
                                      </div>
                                  </div>
                              </div>
                          ))
              ...
              <div className="bg-white rounded-2xl border shadow-sm flex flex-col">
                  <div className="p-4 border-b bg-red-50 flex items-center gap-3 sticky top-0 z-10 rounded-t-2xl">
                      <div className="bg-red-100 p-1.5 rounded-lg text-red-700"><Clock size={16}/></div>
                      <h3 className="font-bold text-sm text-red-900 tracking-wide uppercase">Hutang Supplier Aktif</h3>
                  </div>
                  <div className="p-4 bg-slate-50/50 space-y-3">
                      {dash.hutangBerjalan.length === 0 ? (
                          <div className="text-center p-6 text-slate-400 text-[10px] italic">Tidak ada hutang supplier aktif.</div>
                      ) : (
                          dash.hutangBerjalan.map((h, idx) => (
                              <div key={idx} className="bg-white border border-red-200 rounded-xl p-3 shadow-sm hover:shadow-md transition relative overflow-hidden">
                                  ...
                                  <div className="flex justify-between items-center mb-2 pl-2">
                                      <span className="text-[9px] text-slate-500 font-bold">{formatDate(h.date)} • {h.time}</span>
                                      <span className="text-[9px] font-black bg-slate-100 text-slate-600 px-2 py-0.5 rounded">{h.id}</span>
                                  </div>
                                  <div className="font-black uppercase text-slate-800 text-sm mb-2 pl-2 truncate">{h.supplier}</div>
                                  <div className="flex justify-between items-end pl-2">
                                      <div>
                                          <div className="text-[9px] text-slate-500 uppercase font-bold mb-0.5">Sisa Kewajiban</div>
                                          <div className="font-black text-red-600 text-sm">{formatRp(h.sisaHutang)}</div>
                                      </div>
                                      <div className="text-[9px] font-bold bg-slate-100 text-slate-600 px-2 py-1 rounded border border-slate-200">
                                          Status: Belum Lunas
                                      </div>
                                  </div>
                              </div>
          ...
          {/* KOLOM KANAN: NOTIFIKASI & PRINT MODULAR (Lebar: 3/12) */}
          <div className="lg:col-span-3 flex flex-col gap-6 h-[700px] overflow-y-auto pr-2 custom-scrollbar">
              
              {/* NOTIFIKASI OPERASIONAL */}
              <div className="bg-white rounded-2xl border shadow-sm flex flex-col">
                  <div className="p-4 border-b bg-slate-50 flex items-center gap-3 sticky top-0 z-10 rounded-t-2xl">
                      <div className="bg-white border p-1.5 rounded-lg text-slate-700"><BellRing size={16}/></div>
                      <h3 className="font-bold text-sm text-slate-800 tracking-wide uppercase">Notifikasi Sistem</h3>
                  </div>
                  <div className="p-3 bg-slate-50/50 space-y-2">
                      {dash.alerts.length === 0 ? (
                          <div className="text-center p-6 text-slate-400 text-xs italic">Semua operasional berjalan normal.</div>
                      ) : (
                          dash.alerts.map(a => (
                              <div key={a.id} className={`p-3 rounded-xl border flex items-start gap-3 shadow-sm bg-white ${a.type === 'danger' ? 'border-red-200' : 'border-orange-200'}`}>
                                  ...
                                  <div>
                                      <h4 className={`text-[10px] font-black uppercase mb-0.5 ${a.type === 'danger' ? 'text-red-700' : 'text-orange-700'}`}>{a.title}</h4>
                                      <p className="text-[9px] text-slate-600 leading-snug">{a.desc}</p>
                                  </div>
                              </div>
                          ))
                      )}
              ...
              {/* CETAK LAPORAN MODULAR */}
              <div className="bg-white rounded-2xl border shadow-sm flex flex-col">
                  <div className="p-4 border-b bg-blue-50 flex items-center gap-3 sticky top-0 z-10 rounded-t-2xl">
                      <div className="bg-blue-100 p-1.5 rounded-lg text-blue-700"><Printer size={16}/></div>
                      <h3 className="font-bold text-sm text-blue-900 tracking-wide uppercase">Cetak Modular</h3>
                  </div>
                  <div className="p-4 flex flex-col gap-3 bg-slate-50/50">
                      
                      <div className="border border-slate-200 rounded-xl p-3 bg-white hover:border-blue-300 hover:shadow-md transition">
                          <div className="flex items-center gap-2 mb-1.5"><div className="p-1.5 bg-blue-50 text-blue-600 rounded-lg"><TrendingUp size={14}/></div><h4 className="font-black text-slate-800 text-[10px] uppercase">Lap. Penjualan</h4></div>
                          <button onClick={() => setPrintData({ type: 'report', data: { dash, dateFrom, dateTo, reportType: 'sales' } })} className="w-full bg-slate-100 hover:bg-blue-600 text-slate-700 hover:text-white font-bold py-2 rounded-lg text-[10px] transition uppercase tracking-wide mt-2">Cetak Data</button>
                      </div>

                      <div className="border border-slate-200 rounded-xl p-3 bg-white hover:border-emerald-300 hover:shadow-md transition">
                          <div className="flex items-center gap-2 mb-1.5"><div className="p-1.5 bg-emerald-50 text-emerald-600 rounded-lg"><Wallet size={14}/></div><h4 className="font-black text-slate-800 text-[10px] uppercase">Buku Besar (Ledger)</h4></div>
                          <button onClick={() => setPrintData({ type: 'report', data: { dash, dateFrom, dateTo, reportType: 'finance' } })} className="w-full bg-slate-100 hover:bg-emerald-600 text-slate-700 hover:text-white font-bold py-2 rounded-lg text-[10px] transition uppercase tracking-wide mt-2">Cetak Data</button>
                      </div>
