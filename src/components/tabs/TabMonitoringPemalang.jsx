...
const StatCard = ({ title, amount, icon, colorClass, textClass }) => (
  <div className={`p-5 rounded-3xl shadow-sm flex flex-col justify-between transition-transform hover:scale-[1.02] duration-300 card-holo ${colorClass}`}>
    <div className="flex justify-between items-start mb-3">
      <h3 className={`font-bold text-xs normal-case tracking-wide opacity-85 leading-tight ${textClass}`}>{title}</h3>
...
export default function TabMonitoringPemalang({ orders = [], orders_data, pemalangReports = [], purchases_data, purchases = [], stokData, user }) {
  ...
  const stats = useMemo(() => {
    ...
    // 🔥 FIX TYPO JAVASCRIPT: .tonormal-case() dikembalikan ke .toLowerCase() agar tidak crash
  ...
  return (
    <div className="space-y-6 pb-10 text-slate-700 normal-case">
      ...
      {/* FILTER CONTROL KALENDER - FLAT ENTERPRISE DESIGN */}
      <div className="card-holo p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h3 className="text-xs font-bold text-slate-800 normal-case tracking-wide mb-2 flex items-center gap-1.5"><Calendar size={16} className="text-orange-500"/> Filter rentang pemantauan cabang</h3>
      ...
      {/* MONITOR OPERASIONAL LIVE PANTAUAN PEMALANG - SEAMLESS FLAT LOOK */}
      <div className="card-holo overflow-hidden relative text-slate-700">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 divide-y md:divide-y-0 md:divide-x divide-slate-100 text-center bg-white">
              <div className="p-5 hover:bg-slate-50 transition-colors">
                  <div className="text-[9px] font-bold text-slate-400 normal-case tracking-wide mb-1">Adukan cabang</div>
              ...
              <div className="p-5 hover:bg-slate-50 transition-colors">
                  <div className="text-[9px] font-bold text-slate-400 normal-case tracking-wide mb-1">Daging terpakai</div>
              ...
              <div className="p-5 hover:bg-slate-50 transition-colors bg-slate-50/30 relative">
                  <div className="text-[9px] font-bold text-slate-400 normal-case tracking-wide mb-1">Sisa ayam gudang</div>
                  ...
                  <div className="text-[8px] font-bold text-red-700 bg-red-50 px-1.5 py-0.5 rounded border border-red-200 mt-1 inline-block normal-case tracking-wide">{formatNumber((stats.sisaAyamCabang / KG_PER_KANTONG).toFixed(0))} Kantong</div>
              </div>
              <div className="p-5 hover:bg-slate-50 transition-colors">
                  <div className="text-[9px] font-bold text-slate-400 normal-case tracking-wide mb-1">Masuk freezer</div>
                  ...
                  <div className="text-[8px] font-bold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200 mt-1 inline-block normal-case tracking-wide">{formatNumber((stats.dimsumMasukPeriode / PCS_PER_MIKA).toFixed(0))} Mika</div>
              </div>
              <div className="p-5 hover:bg-slate-50 transition-colors bg-slate-50/30">
                  <div className="text-[9px] font-bold text-slate-400 normal-case tracking-wide mb-1">Sisa freezer (Live)</div>
                  ...
                  <div className="text-[8px] font-bold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200 mt-1 inline-block normal-case tracking-wide">{formatNumber((stats.sisaStokFreezer / PCS_PER_MIKA).toFixed(0))} Mika</div>
              </div>
          </div>
      </div>

      {/* DASHBOARD PERFORMA FINANSIAL PERIODE - CLEAN ENTERPRISE CARD */}
      <div className="card-holo p-6">
          <div className="mb-5 border-b border-slate-100 pb-3">
              <h2 className="text-xs font-extrabold text-slate-800 normal-case tracking-wide flex items-center gap-1.5"><Wallet size={16} className="text-red-500"/> Analitik keuangan buku cabang Pemalang</h2>
      ...
      {/* 🔥 TARGET BRANKAS 4 AMPLOP BERJALAN PEMALANG - FLAT STYLE */}
      {stats.omsetPeriode > 0 && (
         <div className="card-holo p-5 border-l-4 border-l-orange-500 shadow-sm">
           <h3 className="font-extrabold text-xs text-slate-800 normal-case tracking-wide mb-4 flex items-center gap-1.5"><Percent size={14} className="text-orange-500"/> Proyeksi kuota jatah 4 amplop (Dari omset periode ini)</h3>
           <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
              <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-xl"><div className="text-[8px] font-bold text-slate-400 normal-case tracking-wide">Amplop 1: Kas ayam (55%)</div><div className="text-base font-extrabold text-red-600 mt-1">{formatRupiah(stats.jatahAyam55)}</div></div>
              <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-xl"><div className="text-[8px] font-bold text-slate-400 normal-case tracking-wide">Amplop 2: Ops &amp; Gaji (20%)</div><div className="text-base font-extrabold text-blue-600 mt-1">{formatRupiah(stats.jatahOps20)}</div></div>
              <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-xl"><div className="text-[8px] font-bold text-slate-400 normal-case tracking-wide">Amplop 3: Cadangan (10%)</div><div className="text-base font-extrabold text-amber-600 mt-1">{formatRupiah(stats.jatahCadangan10)}</div></div>
              <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-xl"><div className="text-[8px] font-bold text-slate-400 normal-case tracking-wide">Amplop 4: Profit bersih (15%)</div><div className="text-base font-extrabold text-emerald-600 mt-1">{formatRupiah(stats.jatahCuan15)}</div></div>
           </div>
         </div>
      )}
      ...
      <div className="card-holo flex flex-col overflow-hidden">
          <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center gap-2">
             ...
             <h4 className="font-extrabold text-slate-800 text-xs normal-case tracking-wide">Jurnal pengecekan silang setoran (EOD cabang)</h4>
          </div>
              ...
              <table className="w-full text-sm text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 font-bold text-xs">
                      {(!stats.laporanUrut || stats.laporanUrut.length === 0) ? (
                          <tr><td colSpan="5" className="text-center py-10 text-slate-400 normal-case font-bold tracking-wide bg-white">Tidak ada setoran EOD tertulis dari cabang di rentang tanggal ini.</td></tr>
                      ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 card-holo flex flex-col overflow-hidden">
            <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center gap-2">
               ...
               <h3 className="font-extrabold text-slate-800 text-xs tracking-wide normal-case">Buku harian antrean invoice jualan Pemalang</h3>
            </div>
            <div className="overflow-x-auto p-1 custom-scrollbar max-h-[350px]">
                <table className="w-full text-sm text-left border-collapse">
                    <thead className="bg-slate-50 border-b border-slate-200 text-[10px] normal-case text-slate-400 sticky top-0 z-10 shadow-xs bg-white">
                        <tr>
                    ...
                    <tbody className="divide-y divide-slate-100 text-xs font-bold">
                        {(!stats.branchOrdersPeriod || stats.branchOrdersPeriod.length === 0) ? (
                            <tr><td colSpan="4" className="text-center py-10 text-slate-400 normal-case font-bold tracking-wide bg-white">Tidak ada struk transaksi penjualan pada rentang periode ini.</td></tr>
                        ) : (
        ...
        <div className="lg:col-span-5 card-holo p-5 flex flex-col max-h-[415px]">
            <h3 className="font-extrabold text-slate-800 text-xs tracking-wide normal-case flex items-center gap-2 mb-4 border-b border-slate-100 pb-2"><Users size={16} className="text-red-500"/> Klasemen pelanggan terloyal</h3>
            <div className="overflow-y-auto pr-1 flex-1 space-y-2.5 custom-scrollbar">
               {(!stats.topCustomersList || stats.topCustomersList.length === 0) ? (
                   <div className="text-center text-slate-400 normal-case font-bold text-[10px] tracking-wide py-10 bg-slate-50 rounded-xl border border-dashed border-slate-200">Belum ada pelanggan loyal yang masuk hitungan.</div>
               ) : (
