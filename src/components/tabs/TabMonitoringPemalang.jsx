...
const StatCard = ({ title, amount, icon, colorClass, textClass }) => (
  <div className={`p-5 rounded-3xl shadow-sm flex flex-col justify-between transition-transform hover:scale-[1.02] duration-300 card-holo ${colorClass}`}>
    <div className="flex justify-between items-start mb-3">
      <h3 className={`font-black text-xs normal-case tracking-wider opacity-80 leading-tight ${textClass}`}>{title}</h3>
...
export default function TabMonitoringPemalang({ orders = [], orders_data, pemalangReports = [], purchases_data, purchases = [], stokData, user }) {
  ...
  const stats = useMemo(() => {
    ...
    realPurchases.filter(p => !p.isDeleted && p.category === 'BAHAN_BAKU' && String(p.branch_id).tonormal-case() === currentBranch).forEach(p => {
    ...
    const branchOrdersAll = realOrders.filter(o => !o.isDeleted && String(o.branch_id).tonormal-case() === currentBranch);
    ...
    branchOrdersPeriod.forEach(o => {
        const cName = String(o.customer_name || 'UMUM / CASH').tonormal-case();
  ...
  return (
    <div className="space-y-6 animate-in fade-in pb-10 text-slate-800">
      ...
      <div className="card-holo p-5 rounded-3xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h3 className="text-xs font-black text-slate-700 normal-case tracking-widest mb-2 flex items-center gap-1.5"><Calendar size={16} className="text-orange-500"/> Filter Rentang Pemantauan Cabang</h3>
            <div className="flex items-center gap-2 bg-white/50 border border-white/60 px-3 py-1.5 rounded-xl shadow-sm">
              <span className="text-[9px] font-black text-slate-500 normal-case">DARI</span>
              ...
              <span className="text-[9px] font-black text-slate-500 normal-case border-l border-white/60 pl-2">SAMPAI</span>
      ...
      <div className="card-holo rounded-3xl shadow-xl overflow-hidden relative text-slate-800">
          ...
          <div className="p-5 border-b border-white/50 flex justify-between items-center bg-white/40">
              <div>
                  <h2 className="text-base font-black text-slate-800 flex items-center gap-2 tracking-widest normal-case"><Factory className="text-orange-500"/> PANTAUAN LIVE HASIL DAPUR PRODUKSI (PEMALANG)</h2>
                  <p className="text-[10px] font-bold text-slate-500 normal-case mt-1">Monitoring otomatis pergerakan adonan dan isi freezer langsung dari server pusat</p>
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 divide-y md:divide-y-0 md:divide-x divide-white/50 text-center bg-white/20">
              <div className="p-6 hover:bg-white/40 transition-colors">
                  <div className="text-[9px] font-black text-slate-500 normal-case tracking-widest mb-1.5">Adukan Cabang</div>
              ...
              <div className="p-6 hover:bg-white/40 transition-colors">
                  <div className="text-[9px] font-black text-slate-500 normal-case tracking-widest mb-1.5">Daging Terpakai</div>
              ...
              <div className="p-6 hover:bg-white/40 transition-colors bg-white/30 relative">
                  <div className="text-[9px] font-black text-slate-500 normal-case tracking-widest mb-1.5">Sisa Ayam Gudang</div>
                  ...
                  <div className="text-[8px] font-black text-red-700 bg-red-100/80 px-2 py-0.5 rounded-md border border-red-200 mt-2 inline-block normal-case tracking-wider">{formatNumber((stats.sisaAyamCabang / KG_PER_KANTONG).toFixed(0))} Kantong</div>
              </div>
              <div className="p-6 hover:bg-white/40 transition-colors">
                  <div className="text-[9px] font-black text-slate-500 normal-case tracking-widest mb-1.5">Masuk Freezer</div>
                  ...
                  <div className="text-[8px] font-black text-emerald-700 bg-emerald-100/80 px-2 py-0.5 rounded-md border border-emerald-200 mt-2 inline-block normal-case tracking-wider">{formatNumber((stats.dimsumMasukPeriode / PCS_PER_MIKA).toFixed(0))} Mika</div>
              </div>
              <div className="p-6 hover:bg-white/40 transition-colors bg-white/30">
                  <div className="text-[9px] font-black text-slate-500 normal-case tracking-widest mb-1.5">Sisa Freezer (Live)</div>
                  ...
                  <div className="text-[8px] font-black text-emerald-700 bg-emerald-100/80 px-2 py-0.5 rounded-md border border-emerald-200 mt-2 inline-block normal-case tracking-wider">{formatNumber((stats.sisaStokFreezer / PCS_PER_MIKA).toFixed(0))} Mika</div>
              </div>
          </div>
      </div>
      ...
      <div className="card-holo p-6 rounded-3xl">
          <div className="mb-5 border-b border-white/50 pb-3">
              <h2 className="text-xs font-black text-slate-800 normal-case tracking-widest flex items-center gap-1.5"><Wallet size={16} className="text-red-500"/> Analitik Keuangan Buku Cabang Pemalang</h2>
              <p className="text-[10px] font-bold text-slate-500 mt-1 normal-case">Laporan otomatis periode terpilih harian</p>
          </div>
      ...
      {stats.omsetPeriode > 0 && (
         <div className="card-holo p-6 rounded-3xl shadow-lg border-2 border-orange-200/50">
           <h3 className="text-holo-gradient font-black text-xs normal-case tracking-widest mb-4 flex items-center gap-1.5"><Percent size={14} className="text-orange-500"/> Proyeksi Kuota Jatah 4 Amplop (Dari Omset Periode Ini)</h3>
           <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
              <div className="bg-white/50 backdrop-blur-sm p-4 rounded-2xl border border-white/60 shadow-sm"><div className="text-[8px] font-black text-slate-500 normal-case tracking-widest">Amplop 1: Kas Ayam (55%)</div><div className="text-lg font-black text-red-600 mt-1">{formatRupiah(stats.jatahAyam55)}</div></div>
              <div className="bg-white/50 backdrop-blur-sm p-4 rounded-2xl border border-white/60 shadow-sm"><div className="text-[8px] font-black text-slate-500 normal-case tracking-widest">Amplop 2: Ops &amp; Gaji (20%)</div><div className="text-lg font-black text-blue-600 mt-1">{formatRupiah(stats.jatahOps20)}</div></div>
              <div className="bg-white/50 backdrop-blur-sm p-4 rounded-2xl border border-white/60 shadow-sm"><div className="text-[8px] font-black text-slate-500 normal-case tracking-widest">Amplop 3: Cadangan (10%)</div><div className="text-lg font-black text-amber-600 mt-1">{formatRupiah(stats.jatahCadangan10)}</div></div>
              <div className="bg-white/50 backdrop-blur-sm p-4 rounded-2xl border border-white/60 shadow-sm"><div className="text-[8px] font-black text-slate-500 normal-case tracking-widest">Amplop 4: Profit Bersih (15%)</div><div className="text-lg font-black text-emerald-600 mt-1">{formatRupiah(stats.jatahCuan15)}</div></div>
           </div>
         </div>
      )}
      ...
      <div className="card-holo rounded-3xl flex flex-col overflow-hidden">
          <div className="p-5 bg-white/40 border-b border-white/50 flex items-center gap-2">
             ...
             <h4 className="font-black text-slate-800 text-xs normal-case tracking-widest">Jurnal Pengecekan Silang Setoran (EOD Cabang)</h4>
          </div>
          <div className="overflow-x-auto p-2 custom-scrollbar">
              <table className="w-full text-sm text-left border-collapse">
                  <thead className="bg-white/50 border-b border-white/60 text-[10px] text-slate-500 normal-case">
                      <tr>
                  ...
                  <tbody className="divide-y divide-white/40 font-bold text-xs">
                      {(!stats.laporanUrut || stats.laporanUrut.length === 0) ? (
                          <tr><td colSpan="5" className="text-center py-12 text-slate-500 normal-case font-black tracking-widest bg-white/20">Tidak ada setoran EOD tertulis dari cabang di rentang tanggal ini.</td></tr>
                      ) : (
                          stats.laporanUrut.map((r, i) => (
                              <tr key={i} className="hover:bg-white/40 transition-colors">
                                  ...
                                  <td className="px-5 py-4 font-black normal-case text-indigo-700">{formatNumber(r.stokFreezer)} PCS</td>
                                  <td className="px-5 py-4 font-black normal-case text-orange-700">{formatNumber(r.stokAyam)} KG</td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 card-holo rounded-3xl flex flex-col overflow-hidden">
            <div className="p-5 bg-white/40 border-b border-white/50 flex items-center gap-2">
               ...
               <h3 className="font-black text-slate-800 text-xs tracking-widest normal-case">Buku Harian Antrean Invoice Jualan Pemalang</h3>
            </div>
            <div className="overflow-x-auto p-2 custom-scrollbar max-h-[350px]">
                <table className="w-full text-sm text-left border-collapse">
                    <thead className="bg-white/60 border-b border-white/70 text-[10px] normal-case text-slate-500 sticky top-0 z-10 shadow-sm backdrop-blur-md">
                        <tr>
                    ...
                    <tbody className="divide-y divide-white/40 text-xs font-bold">
                        {(!stats.branchOrdersPeriod || stats.branchOrdersPeriod.length === 0) ? (
                            <tr><td colSpan="4" className="text-center py-12 text-slate-500 normal-case font-black tracking-widest bg-white/20">Tidak ada struk transaksi penjualan pada rentang periode ini.</td></tr>
                        ) : (
                            stats.branchOrdersPeriod.map((o, i) => {
                                ...
                                return (
                                    <tr key={i} className="hover:bg-white/50 transition-colors">
                                        ...
                                        <td className="px-4 py-4 normal-case font-black text-xs text-slate-700 whitespace-nowrap">{o?.customer_name || '-'}</td>
        ...
        <div className="lg:col-span-5 card-holo p-6 rounded-3xl flex flex-col max-h-[415px]">
            <h3 className="font-black text-slate-800 text-xs tracking-widest normal-case flex items-center gap-2 mb-4 border-b border-white/50 pb-3"><Users size={16} className="text-red-500"/> Klasemen Pelanggan Terloyal</h3>
            <div className="overflow-y-auto pr-1 flex-1 space-y-3 custom-scrollbar">
               {(!stats.topCustomersList || stats.topCustomersList.length === 0) ? (
                   <div className="text-center text-slate-500 normal-case font-black text-[10px] tracking-widest py-12 bg-white/40 rounded-2xl border border-dashed border-white/60">Belum ada pelanggan loyal yang masuk hitungan.</div>
               ) : (
                   stats.topCustomersList.map((cust, i) => (
                       <div key={i} className="flex items-center justify-between p-3 bg-white/50 backdrop-blur-sm rounded-2xl border border-white/60 hover:border-orange-300 hover:bg-orange-50/30 transition-colors shadow-sm group">
                           <div className="flex items-center gap-3">
                               ...
                               <div>
                                 <div className="font-black text-slate-800 text-xs normal-case line-clamp-1 group-hover:text-red-600 transition-colors">{cust.name}</div>
                                 <div className="text-[9px] font-bold text-slate-500 normal-case mt-0.5">{cust.frequency}x Belanja • {formatNumber(cust.qty)} Pcs ({formatNumber(cust.porsi.toFixed(0))} Porsi)</div>
                               </div>
                           </div>
