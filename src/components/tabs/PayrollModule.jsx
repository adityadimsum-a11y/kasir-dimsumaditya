    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 normal-case">
      <div className={`p-6 rounded-3xl border shadow-sm transition-all h-max ${isEditing ? 'bg-amber-50/50 border-amber-300' : 'bg-white border-slate-200'}`}>
        <form onSubmit={async (e) => {
          ...
          if (form.isProrata) detailDesc += ` (Prorata Absen: Masuk ${form.hariHadir}/${form.hariNormal} Hari. Potong: Rp ${totalMinusProrata})`;
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-black text-slate-400 normal-case flex justify-between items-center block mb-1">Gaji pokok {form.isProrata && <span className="text-[8px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded normal-case font-black">Auto</span>}</label>
              ...
              {form.isProrata && totalMinusProrata > 0 && (<div className="text-[8px] font-black text-rose-500 mt-1.5 normal-case">Kepotong: -{formatRupiah(totalMinusProrata)}</div>)}
            </div>
            <div><label className="text-[10px] font-black text-blue-600 normal-case block mb-1">Bonus/tunjangan</label><input type="text" required value={formatRupiah(form.allowance)} onChange={e=>setForm({...form, allowance: e.target.value.replace(/\D/g, '')})} className="w-full p-3 border border-blue-200 rounded-xl font-black text-sm bg-blue-50/50 outline-none focus:bg-white focus:border-blue-500 transition-colors" /></div>
          </div>
          
          <div className="bg-orange-50 p-4 rounded-2xl border border-orange-200 shadow-inner">
            <div className="flex justify-between items-end mb-2"><label className="text-[10px] font-black text-orange-800 normal-case flex items-center gap-1.5"><History size={14}/> Potong cicilan kredit / kasbon</label><span className="text-[9px] font-black text-rose-600 bg-rose-100 px-2 py-1 rounded-lg border border-rose-200 normal-case">Hutang aktif: {formatRupiah(sisaHutangReal)}</span></div>
          ...
          <div className="bg-slate-900 text-white p-5 rounded-3xl shadow-lg border border-slate-800 relative overflow-hidden mt-2">
            ...
            <div className="text-[9px] text-emerald-400 normal-case font-black flex items-center gap-1.5 mb-1"><CheckCircle2 size={12}/> Total gaji bersih (Take Home Pay)</div>
            <div className="text-3xl font-black tracking-tight">{formatRupiah(hitungNetto.totalCair)}</div>
          </div>
          <button type="submit" className={`w-full text-white font-black py-4 rounded-xl text-xs normal-case shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2 mt-2 ${isEditing ? 'bg-amber-500 hover:bg-amber-600' : 'bg-slate-800 hover:bg-slate-900'}`}>{isEditing ? '💾 Update Lembar Gaji' : 'Cairkan & potong kas dompet'}</button>
        </form>
      </div>
      
      <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-200 overflow-hidden flex flex-col shadow-sm">
        <div className="p-5 bg-slate-50 border-b border-slate-100 font-black text-xs normal-case text-slate-700 flex items-center gap-2"><FileText size={16} className="text-blue-500"/> Histori penggajian {activeBranch === 'SEMUA_CABANG' ? 'Nasional (Gabungan)' : `Area ${activeBranch.replace('_', ' ')}`}</div>
          ...
          <table className="w-full text-sm text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50 text-xs font-bold">
              {historyGaji.length === 0 ? (
                 <tr><td colSpan="6" className="text-center py-20 text-slate-400 font-black normal-case bg-slate-50/50">Belum ada riwayat penggajian tercatat pada buku ini.</td></tr>
              ) : (
                historyGaji.map(p => {
                  const emp = globalCompiled[p.employee_id];
                  const isDescPeriod = p.description && p.description.toUpperCase().includes('PERIODE:');
                  const extractedPeriod = isDescPeriod ? p.description.split(/Periode:/i)[1].trim().split('(')[0] : formatDate(p.date);
                  const isProrataLog = p.description && p.description.toUpperCase().includes('PRORATA');
                  const stringProrataCut = isProrataLog && p.description.match(/Potong: Rp (\d+)/i);
                  const valProrata = stringProrataCut ? Number(stringProrataCut[1]) : 0;

                  return (
                    <tr key={p.id} className="hover:bg-blue-50/30 transition-colors group">
                      <td className="px-5 py-4 whitespace-nowrap"><div className="text-slate-800 font-black text-sm normal-case">{extractedPeriod}</div><div className="text-[9px] font-mono text-slate-400 mt-1">{p.id}</div></td>
                      <td onClick={() => emp && onViewDetails(emp)} className="px-5 py-4 flex items-center gap-3 cursor-pointer">
                        ...
                        <div><span className="normal-case font-black text-slate-800 group-hover:text-blue-600 transition-colors block">{emp?.name || 'Staf karyawan'}</span>{activeBranch === 'SEMUA_CABANG' && <span className="text-[8px] font-black normal-case text-slate-400 mt-1 block">Lokasi node: {emp?.branch_id.replace('_', ' ')}</span>}</div>
                      </td>
