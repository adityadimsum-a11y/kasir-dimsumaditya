    ...
    <div className="space-y-6 animate-in fade-in pb-10 text-slate-700 normal-case">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {isHQ ? (
          <div className="card-holo p-6 md:col-span-2 grid grid-cols-2 gap-4 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5"><Wallet size={14} className="text-emerald-500"/> Anggaran Amplop Gaji (20%)</div>
              <div className="text-2xl font-extrabold tracking-tight mt-1 text-slate-800">{formatRupiah(metrikSDM.budgetGaji)}</div>
              <div className="text-[10px] mt-2 font-medium text-slate-400 uppercase tracking-widest">Gaji Terbayar: <span className="text-red-500 font-bold">{formatRupiah(metrikSDM.gajiGlobal)}</span></div>
            </div>
            <div className="relative z-10 border-l border-slate-100 pl-4">
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5"><ArrowRightLeft size={14} className="text-blue-500"/> Sisa Anggaran HQ</div>
              ...
              <span className={`text-[9px] mt-2 inline-block px-2.5 py-0.5 rounded-md font-bold uppercase tracking-widest border shadow-xs ${metrikSDM.sisaNafasAnggaran >= 0 ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-700 border-red-200 animate-pulse'}`}>{metrikSDM.statusAnggaran}</span>
            </div>
          </div>
        ) : (
          <div className="card-holo p-6 md:col-span-2 flex flex-col justify-center border-l-4 border-l-blue-500">
            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Total Gaji Terbayar (Bulan Ini)</div>
        ...
        <div className="card-holo p-6 flex flex-col justify-center border-l-4 border-l-orange-500">
          <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Piutang Kasbon (Nasional)</div>
        ...
        <div className="card-holo p-6 flex flex-col justify-center border-l-4 border-l-emerald-500">
          <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Kasbon Area Aktif</div>
      ...
      {isHQ && (
        <div className="card-holo p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-2"><Layers size={16} className="text-blue-600" /><span className="text-[10px] font-bold uppercase tracking-widest text-slate-600">Pilih Radar Cabang SDM:</span></div>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => setSelectedBranchFilter('SEMUA_CABANG')} className={`px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${activeProcessingBranch === 'SEMUA_CABANG' ? 'bg-white border border-slate-200 shadow-xs text-red-600' : 'bg-transparent text-slate-500 hover:bg-slate-50'}`}>🌍 Nasional (Gabungan)</button>
            {daftarCabangId.map(brId => (
              <button key={brId} type="button" onClick={() => setSelectedBranchFilter(brId)} className={`px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${activeProcessingBranch === brId ? 'bg-white border border-slate-200 shadow-xs text-red-600' : 'bg-transparent text-slate-500 hover:bg-slate-50'}`}>{petaNamaCabang[brId]}</button>
            ))}
          </div>
        </div>
      )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="text-xs font-semibold text-slate-600 space-y-2.5 normal-case">
                ...
                <div className="md:col-span-2 space-y-6">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-xs">
                          ...
                          <div key={i} className={`border p-4 rounded-xl relative overflow-hidden transition-all shadow-xs ${kr.status === 'Lunas' ? 'bg-emerald-50/50 border-emerald-200' : 'bg-white border-slate-200 hover:border-blue-300'}`}>
                            ...
                            <div className="flex gap-3">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="text-[9px] text-slate-400 mb-2 font-medium normal-case">{formatDate(kr.date)} | ID: {kr.id}</div>
                    ...
                    ) : (<div className="text-center py-6 text-[10px] font-bold text-slate-400 bg-white rounded-xl border border-dashed border-slate-200 normal-case">Belum ada riwayat kredit barang / cicilan.</div>)}
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-3xl p-5 shadow-xs">
                        ...
                        <div className="space-y-3">
                          {selectedEmployeeDetails.history_kasbon.map((ks, i) => (
                            <div key={i} className={`flex flex-col border border-slate-200 p-3.5 rounded-xl gap-2 shadow-xs ${ks.status === 'Lunas' ? 'bg-slate-100/50' : 'bg-white border-l-4 border-l-orange-500'}`}>
                              ...
                              <div className="flex justify-between items-center text-[10px] font-semibold border-t border-slate-100 pt-2 mt-1 normal-case">
                      ...
                      ) : (<div className="text-center py-6 text-[10px] font-bold text-slate-400 bg-white rounded-xl border border-dashed border-slate-200 normal-case">Tidak ada hutang kasbon aktif.</div>)}
                    </div>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-3xl p-5 shadow-xs">
                          ...
                          {selectedEmployeeDetails.payroll_list.map((pr, i) => {
                            ...
                            return (
                              <div key={i} className="flex flex-col justify-between bg-white border border-slate-200 p-4 rounded-xl shadow-xs hover:border-emerald-300 transition-colors">
                                <div><div className="text-xs font-extrabold text-slate-800 normal-case border-b border-slate-100 pb-2 mb-2">Periode: {extractedPeriod}</div><div className="text-[9px] text-slate-400 font-medium normal-case">Tgl Cair: {formatDate(pr.date)}</div></div>
                                <div className="mt-3 pt-3 border-t border-dashed border-slate-200 space-y-1 normal-case">
                      ...
                      ) : (<div className="text-center py-6 text-[10px] font-bold text-slate-400 bg-white rounded-xl border border-dashed border-slate-200 normal-case">Belum ada riwayat penggajian.</div>)}
                    </div>
                  </div>
                </div>
              </div>
