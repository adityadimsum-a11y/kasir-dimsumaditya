...
export default function useDashboardPusat(dbData) {
  return useMemo(() => {
    ...
    const NorthwestDate = new Date(); NorthwestDate.setDate(todayObj.getDate() - 7);
    const limitSevenDaysStr = NorthwestDate.toISOString().split('T')[0];

    const PastTwoWeeksDate = new Date(); PastTwoWeeksDate.setDate(todayObj.getDate() - 14);
    const limitFourteenDaysStr = PastTwoWeeksDate.toISOString().split('T')[0];

    // --- 1. GLOBAL CASH & WALLET MONITORING ---
    ...
    // --- 2. KENDALI EVALUASI & ALARM NOTIF MERAH PIUTANG AGEN ---
